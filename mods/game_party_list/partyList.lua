...
function toggle()
  if partyList:isVisible() then
    ...
    if m_interface.addToPanels(partyList) then
      ...
      if filterPanel and filterBattleButton and not filterPanel:isVisible() then
...
function onPlayerLoad(config)
  ...
  for _, value in pairs(config.battleListFilters) do
    ...
    PartyClass.panel:setFilter(value, true)
  end
  ...
  for _, value in pairs(config.battleListFilters) do
    ...
    if button then
    PartyClass.panel:setFilter(invertedValue, false)
...
function setupPartyPanel(showFilters)
  ...
  if not showFilters then
    ...
    if filterBattleButton then filterBattleButton:setOn(false) end
    hideFilterPanel()
  else
    if filterPanel:isVisible() then
      return
    end
    if filterBattleButton then filterBattleButton:setOn(true) end
    showFilterPanel()
  end
end
...
function getUpcomingPartyMembers()
  ...
  local players = {}
  for _, creature in pairs(UICreatureButtonList.getPartyCreatures(PartyClass.panel)) do
