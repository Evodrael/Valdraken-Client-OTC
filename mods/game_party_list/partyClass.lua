...
function PartyClass:setup(windowId, window)
  if not window then
    PartyClass.window = g_ui.createWidget('partyList', m_interface.getContainerPanel())
  ...
  partyPanel.createButton = function()
    local battleButton = g_ui.createWidget('BattleButton', partyPanel) or g_ui.createWidget('CreatureButton', partyPanel)
    if not battleButton then return end
    UICreatureButton.setup(battleButton)
    if battleButton.manaBarWidget then
      battleButton.manaBarWidget:setVisible(true)
    end
  ...
  UICreatureButtonList.startCreatureRefresh(PartyClass.panel)
  ...
  PartyClass.window.onMouseRelease = function(widget, mousePos, mouseButton)
    if mouseButton == MouseRightButton and not g_mouse.isPressed(MouseLeftButton) then
      ...
      if child and child:getClassName() == "UICreatureButton" then
        return
      end
