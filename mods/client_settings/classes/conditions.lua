...
local function refreshStatusIconBar()
    if StatusIconBar and StatusIconBar.scheduleRefresh then
        StatusIconBar.scheduleRefresh()
    end
end

local function isConditionVisibleInHudStack(condition)
    if not condition then
        return false
    end

    return (condition:isVisibleHud() and m_settings.getOption('showInHudCheckBox')) or
        (condition:isVisibleBar() and m_settings.getOption('showInBarCheckBox'))
end
...
function ConditionsHUD:setShowInHudEnabled(value)
    ...
    for _, condition in pairs(ConditionsHUD.hud) do
        ...
        if widget then
            ...
            if localPlayer then
                -- addHUDCondition / removeHUDCondition control the on-screen
                -- HUD icons over the player. The Bar (StatusIconBar) is a
                -- separate render path. So this toggle must ONLY look at the
                -- HUD side — not "HUD or Bar" via isConditionVisibleInHudStack.
                -- Otherwise unchecking "Show in HUD" with "Show in Bar" still
                -- on keeps every condition visible on the HUD.
                local active = ConditionsHUD.actives[condition:getId()]
                if value and active and condition:isVisibleHud() then
                    localPlayer:addHUDCondition(condition:getId())
...
function ConditionsHUD:setShowInBarEnabled(value)
    ...
    for _, condition in pairs(ConditionsHUD.hud) do
        ...
        if localPlayer then
            -- Bar visibility is rendered by StatusIconBar (see refreshIcons
            -- in modules/game_healthcircle/statusiconbar.lua). The HUD render
            -- path uses LocalPlayer:add/removeHUDCondition. Toggling the Bar
            -- must NOT touch the HUD render set — only the global "Show in HUD"
            -- toggle (and the per-condition isVisibleHud flag) does that.
            local active = ConditionsHUD.actives[condition:getId()]
            local showInHud = m_settings.getOption('showInHudCheckBox')
            if showInHud and active and condition:isVisibleHud() then
                localPlayer:addHUDCondition(condition:getId())
            else
                localPlayer:removeHUDCondition(condition:getId())
            end
...
function ConditionsHUD:notifierStatesChange(localPlayer, now, old, statesList, removedStates)
    ...
    for _, state in pairs(statesList) do
        -- Removed: `localPlayer:lockWalk(2000)` for paralyze. This fired on EVERY
        -- state-change notification (PZ entry/exit, any condition tick, any new
        -- icon) while paralyze was still active, locking walk for 2s each time.
        -- That was the "2 second freeze" the user kept reporting — companion to
        -- the same buggy lockWalk(2000) that lived in game_healthinfo. Paralyze
        -- pacing belongs to the server's stepDuration, not to a client-side icon
        -- cooldown.
        local specialCondition = ConditionsHUD:getSpecialConditionById(Icons[state].id)
        if specialCondition then
            ...
            if not ConditionsHUD.actives[specialCondition:getId()] then
                ...
                if isConditionVisibleInHudStack(specialCondition) then
...
local DAILY_REWARD_SERVER_STREAK_OFFSET = 7
local DAILY_REWARD_RESTING_BONUS_NAMES = {
...
local function getDailyRewardDisplayStreak(dayStreakLevel)
    dayStreakLevel = tonumber(dayStreakLevel) or 0
    if dayStreakLevel >= DAILY_REWARD_SERVER_STREAK_OFFSET then
        return dayStreakLevel - DAILY_REWARD_SERVER_STREAK_OFFSET
    end

    return dayStreakLevel
end

local function getDailyRewardRestingAreaMessage(streak)
    ...
    local bonuses = {}
    table.insert(bonuses, streak >= 4 and DAILY_REWARD_RESTING_BONUS_NAMES[4] or DAILY_REWARD_RESTING_BONUS_NAMES[1])
    if streak >= 2 then
        table.insert(bonuses, streak >= 5 and DAILY_REWARD_RESTING_BONUS_NAMES[5] or DAILY_REWARD_RESTING_BONUS_NAMES[2])
    end
    if streak >= 3 then
        table.insert(bonuses, DAILY_REWARD_RESTING_BONUS_NAMES[3])
    end
    if streak >= 6 then
        table.insert(bonuses, DAILY_REWARD_RESTING_BONUS_NAMES[6])
    end

    return "Active Resting Area Bonuses:\n" .. table.concat(bonuses, ",\n") .. "."
end

local function getDailyRewardRestingAreaState(state, message)
    local displayStreak = ConditionsHUD and ConditionsHUD.dailyRewardDisplayStreak or rawget(_G, 'DailyRewardDisplayStreak')
    local displayMessage = ConditionsHUD and ConditionsHUD.dailyRewardRestingAreaMessage or rawget(_G, 'DailyRewardRestingAreaMessage')
    if type(displayStreak) == 'number' then
        return displayStreak >= 1 and 1 or 0, displayMessage or message
    end

    return tonumber(state) or 0, message
end

function ConditionsHUD:setDailyRewardRestingAreaState(dayStreakLevel)
    local displayStreak = getDailyRewardDisplayStreak(dayStreakLevel)
    ConditionsHUD.dailyRewardDisplayStreak = displayStreak
    ConditionsHUD.dailyRewardRestingAreaMessage = getDailyRewardRestingAreaMessage(displayStreak)

    if ConditionsHUD.zone ~= nil then
        ConditionsHUD:notifierRestingAreaState(ConditionsHUD.zone, displayStreak >= 1 and 1 or 0, ConditionsHUD.dailyRewardRestingAreaMessage)
    end
end

function ConditionsHUD:notifierRestingAreaState(zone, state, message)
    state, message = getDailyRewardRestingAreaState(state, message)
