...
function init()
  ...
  mouseGrabberSetting:setVisible(true)
  ...
  -- HD: aplica a escala JA no LOAD DO DAT (startup), antes das texturas serem construidas, p/
  -- elas nascerem no fator de HD -> SEM re-bake assincrono no 1o login (que causava o flicker
  -- azul de ~1s). Aplicar so no online()/setup() chegava tarde demais. Tambem aplica na hora
  -- se o dat ja estiver carregado quando este modulo inicia.
  connect(g_things, { onLoadDat = applyHdScaleEarly })
...
-- HD (xBRZ, soyfabi): aplica o fator de HD persistido JA no LOAD DO DAT (startup), antes do
-- atlas/texturas serem construidos, p/ getSpriteSize() ja nascer com base*fator (sem reload).
-- Idempotente (setScaleFactor faz early-return se o fator nao mudou).
function applyHdScaleEarly()
  if g_sprites and g_sprites.setScaleFactor then
    g_sprites.setScaleFactor(g_settings.getBoolean("hdModeBox") and 2 or 1)
  end
end

-- ===== Botao de MUTAR/DESMUTAR o som do jogo (tela inicial, topo) =====
-- API: g_sounds.setAudioEnabled(bool) / isAudioEnabled(). Estado persistido em g_settings
-- ('audioMuted') e aplicado no init (vale na tela inicial e em todo o jogo).
function updateAudioButtonIcon()
  if not audioButton or not (g_sounds and g_sounds.isAudioEnabled) then return end
  local enabled = g_sounds.isAudioEnabled()
  audioButton:setIcon(enabled and '/images/topbuttons/audio' or '/images/topbuttons/audio_mute')
  audioButton:setTooltip(enabled and tr('Mute sound') or tr('Unmute sound'))
end

function toggleAudioMute()
  if not (g_sounds and g_sounds.setAudioEnabled) then return end
  local enable = not g_sounds.isAudioEnabled()
  g_sounds.setAudioEnabled(enable)
  g_settings.set('audioMuted', not enable)
  updateAudioButtonIcon()
end

function applyAudioMuteFromSettings()
  if not (g_sounds and g_sounds.setAudioEnabled) then return end
  g_sounds.setAudioEnabled(not g_settings.getBoolean('audioMuted'))
  updateAudioButtonIcon()
end
...
function online()
  ...
  tmpResetActions = {}
  -- O botao de mutar som existe SO na tela inicial -> esconde ao logar no personagem.
  if audioButton then audioButton:hide() end
  local antialiasingMode = GameOptions:getOption("antialiasing")
  local effectiveAntialiasingMode = antialiasingMode
  local gameMapPanel = m_interface.getMapPanel()
  g_app.setSmooth(antialiasingMode == 2)

  -- HD (xBRZ, soyfabi): reaplica o fator de HD persistido no onGameStart (backup do onLoadDat),
  -- lendo direto de g_settings (default ja registado em setupStart no init). Idempotente:
  -- setScaleFactor faz early-return se o fator nao mudou.
  if g_sprites and g_sprites.setScaleFactor then
    g_sprites.setScaleFactor(g_settings.getBoolean("hdModeBox") and 2 or 1)
  end

  if gameMapPanel then
    gameMapPanel:setAntiAliasingMode(math.max(0, effectiveAntialiasingMode - 1))
  end
...
function offline()
  -- Volta a mostrar o botao de mutar som ao sair p/ a tela inicial.
  if audioButton then audioButton:show(); updateAudioButtonIcon() end
  ...
  if selectedWindow.children then
    ...
    for i, option in pairs(options) do
      if option.parent and option.parent == selectedWindow:getId() then
        ...
        widget:setId(option.id)
        if option.id == 'voip' then
          widget.button:setImageSource("/images/optionstab/voip")
        else
          widget.button:setImageSource("/images/optionstab/" .. option.id)
        end
...
function configureGeneralHotkeys(searchText)
  ...
  for _, action in ipairs(sortedActions) do
    ...
    for option, info in pairs(options) do
      ...
      setStringColor(t, short_text(option, 29), "#c0c0c0")

      widget.action:setColoredText(string.fromColoredTable(t))
...
function resetHud()
  ...
  local yesFunction = function()
    ...
    setTempOption('sizeBox', 2)
    setTempOption('opacityArc', 70)
    setTempOption('distanceArc', 15)
...
function resetSound()
  setTempOption('masterVolumeScrollBar', 100)
  setTempOption('musicVolumeScrollBar', 100)
  setTempOption('anthemMusic', true)
  setTempOption('ambienceVolumeScrollBar', 100)
  setTempOption('itemVolumeScrollBar', 100)
  setTempOption('foodBeverages', true)
  setTempOption('moveItemMusic', true)
  setTempOption('eventVolumeScrollBar', 100)
  onApplyOptions()
end

function resetBattleSounds()
  setTempOption('ownBattleVolumeScrollBar', 100)
  setTempOption('otherBattleVolumeScrollBar', 100)
  setTempOption('creatureBattleVolumeScrollBar', 100)
  setTempOption('ownSpellSound', true)
  setTempOption('ownAttackSound', true)
  setTempOption('ownHealingSound', true)
  setTempOption('ownSupportSound', true)
  setTempOption('ownWeaponsSound', true)
  setTempOption('otherSpellSound', true)
  setTempOption('otherAttackSound', true)
  setTempOption('otherHealingSound', true)
  setTempOption('otherSupportSound', true)
  setTempOption('otherWeaponsSound', true)
  setTempOption('creatureNoiseSound', true)
  setTempOption('creatureDeathSound', true)
  setTempOption('creatureSpellSound', true)
  onApplyOptions()
end

function resetUiSounds()
  setTempOption('uiVolumeScrollBar', 100)
  setTempOption('uiSounds', true)
  setTempOption('partySounds', true)
  setTempOption('vipSounds', true)
  setTempOption('consoleMessageSounds', true)
  setTempOption('partyMessageSounds', true)
  setTempOption('guildMessageSounds', true)
  setTempOption('privateLocalMessageSounds', true)
  setTempOption('privateMessageSounds', true)
  setTempOption('npcMessageSounds', true)
  setTempOption('globalMessageSounds', true)
  setTempOption('finderMessageSounds', true)
  setTempOption('raidMessageSounds', true)
  setTempOption('systemMessageSounds', true)
  onApplyOptions()
end
...
function onOpenRewardWall(fromShrine, nextRewardTime, currentIndex, message, dailyState, jokerToken, serverSave, dayStreakLevel)
  ConditionsHUD:setDailyRewardRestingAreaState(dayStreakLevel)
end
...
local function getVoipNative()
  return rawget(_G, 'g_voip')
end

local function callVoip(method, ...)
  local native = getVoipNative()
  if native and native[method] then
    local ok, result = pcall(native[method], ...)
    if ok then
      return result
    end
  end
...
local function normalizeDeviceList(devices)
  local list = {}
  if type(devices) == 'table' then
    for _, device in pairs(devices) do
      if type(device) == 'table' then
        list[#list + 1] = device.name or device[1] or tostring(device.id or #list)
      elseif device ~= nil then
        list[#list + 1] = tostring(device)
      end
    end
  end

  if #list == 0 then
    list[1] = "Default device"
  end
  return list
end

function setupVoipDeviceCombo(widget, kind)
  if not widget then
    return
  end

  local devices = kind == 'output' and callVoip('listOutputDevices') or callVoip('listInputDevices')
  ...
  local optionName = kind == 'output' and 'speakerVoip' or 'microphoneVoip'
  local currentIndex = tonumber(getTmpOption(optionName) or getOption(optionName) or 1) or 1
  currentIndex = math.max(1, math.min(currentIndex, #devices))
  widget:setCurrentIndex(currentIndex, true)
end

function setupVoipPanel()
  local panel = loadedWindows and loadedWindows["voip"]
  if not panel then
    return
  end

  setupVoipDeviceCombo(panel:recursiveGetChildById('microphoneVoip'), 'input')
  setupVoipDeviceCombo(panel:recursiveGetChildById('speakerVoip'), 'output')

  local voiceActivity = panel:recursiveGetChildById('voiceActivity')
  if voiceActivity then
    voiceActivity:setPercent(0)
  end
end

function refreshVoipDevices()
  setupVoipPanel()
end

function testVoipAudio()
  local native = getVoipNative()
  if not native then
    modules.game_textmessage.displayFailureMessage("VoIP: suporte nativo indisponivel.")
    return
  end

  if native.startMicTest and native.stopMicTest then
    native.startMicTest()
    scheduleEvent(function()
      if native.stopMicTest then
        native.stopMicTest()
      end
    end, 1500)
  else
    modules.game_textmessage.displayFailureMessage("VoIP: entre em uma sala de voz para testar o microfone.")
  end
end
...
function updateVoiceActivity(level)
  ...
  local voiceActivity = panel:recursiveGetChildById('voiceActivity')
  if voiceActivity then
    voiceActivity:setPercent(math.max(0, math.min(100, tonumber(level) or 0)))
  end
end
