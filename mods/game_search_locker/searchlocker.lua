...
-- Define o nome do item com truncamento seguro. isOfflimit/isTextWraped NAO existem em todas as
-- builds do widget (eram nil -> crash em insertWidget abortando o loop = "so 1 item"). Guardamos a
-- chamada como o game_tibia_market faz.
local function updateItemName(widget, fullName)
    ...
    if isOfflimit or #fullName > 15 then
...
local function canShowItem(itemInfo)
    ...
    local count = getLockerItemCount(itemInfo.thingType:getId(), tier) -- usa o tier selecionado
...
local function insertWidget(itemInfo, itemList)
    ...
    updateItemName(widget, itemInfo.marketData.name)
...
function onSelectChildCategory(widget, selected, resetFilter)
    ...
    if showLockerOnly then
        ...
        for k = 1, #titemList do
            ...
            local count = getLockerItemCount(itemInfo.thingType:getId(), sortButtons["tierFilter"])
            if count > 0 then
                listConfig.max = listConfig.max + 1
...
function checkSortLockerOptions(itemData)
    ...
    if currentOption.text ~= 'All Traders' then
        -- hasNpcSale pode nao existir nesta build do ThingType (era nil -> crash); se faltar, nao
        -- filtra por vendedor (mostra todos).
        if itemData.thingType and itemData.thingType.hasNpcSale
            and not itemData.thingType:hasNpcSale(currentOption.text) then
            return false
        end
    end
    return true
