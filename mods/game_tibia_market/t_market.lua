...
local function sanitizeMarketData(itemType)
	local marketData = table.copy(itemType:getMarketData() or {})
	if table.empty(marketData) then
		return nil
	end

	if not marketData.category or marketData.category < MarketCategory.First or marketData.category > MarketCategory.WeaponsAll then
		return nil
	end
...
local function updateMarketItemName(widget, fullName)
	if not widget or not widget.name then
		return
	end

	widget.name:setTooltip('')
	widget.name:setText(fullName)

	local isOfflimit = widget.name.isOfflimit and widget.name:isOfflimit()
	local isWrapped = widget.name.isTextWraped and widget.name:isTextWraped()
	local shouldShorten = isOfflimit or #fullName > 15
	if shouldShorten then
		widget.name:setText(short_text(fullName, 15))
		widget.name:setTooltip(fullName)
	end

	if not isWrapped and #widget.name:getText() <= 15 then
		widget.name:setMarginTop(1)
	end
end

local function setMarketItemTier(itemWidget, itemTier)
	itemTier = tonumber(itemTier or 0) or 0
	...
	if itemWidget.setTier then
		itemWidget:setTier(itemTier)
	end

	if ItemsDatabase and ItemsDatabase.setTier then
		ItemsDatabase.setTier(itemWidget, itemTier, true)
	end

	local tierWidget = itemWidget:getChildById('tier')
	if tierWidget then
		tierWidget:setVisible(itemTier > 0)
	end
end

local function applyRarityFrame(itemWidget)
	if itemWidget and ItemsDatabase and ItemsDatabase.setRarityItem then
		ItemsDatabase.setRarityItem(itemWidget, itemWidget:getItem())
	end
end

local function updateMarketItemBackground(widget, itemTier)
	if not widget or not widget.itemBackground then
		return
	end

	itemTier = tonumber(itemTier or 0) or 0
	widget.itemBackground:setImageSource('/images/ui/item')
end

local function updateDepotCountLabel(widget, count)
	if widget and widget.depotCount then
		widget.depotCount:setText(count > 0 and comma_value(count) or "")
	end
end
...
function show()
	g_client.setInputLockWidget(marketWindow)
	marketWindow:show(true)
	if marketWindow.contentPanel and marketWindow.contentPanel.searchText then
		marketWindow.contentPanel.searchText:focus()
	end
end
...
local function getSelectedMarketItemId()
  return (lastSelectedItem and lastSelectedItem.itemId) or lastItemID or 0
end

local function getSelectedMarketItemTier()
  if lastSelectedItem and lastSelectedItem.tier ~= nil then
    return lastSelectedItem.tier
  end
  return lastItemTier or 0
end
...
function offersButton()
  ...
  if not mainMarket:isVisible() then
    local itemId = getSelectedMarketItemId()
    if itemId > 0 then
      g_game.sendMarketAction(3, itemId, getSelectedMarketItemTier())
    end
    detailsMarket:setVisible(false)
    mainMarket:setVisible(true)
    marketButton:setVisible(false)
...
local function getMarketEntryTier(entry)
	if not entry then
		return sortButtons["tierFilter"] or 0
	end

	if entry.tier ~= nil then
		return entry.tier
	end
...
function onUpdateResourceValue(resourceType, resourceValue, balanceOverride)
	if not g_game.isOnline() or not marketWindow or not marketWindow:isVisible() then
		return
	end
	if balanceOverride == nil and resourceType ~= nil and resourceType ~= ResourceBank and resourceType ~= ResourceInventary then
		return
	end
	...
	local balance = tonumber(balanceOverride)
...
function configureList()
	...
	marketItems[MarketCategory.Gold] = marketItems[MarketCategory.Gold] or {}
	marketItems[MarketCategory.Unassigned] = marketItems[MarketCategory.Unassigned] or {}
	marketItems[MarketCategory.WeaponsAll] = marketItems[MarketCategory.WeaponsAll] or {}

	local types = (g_things.getMarketThings and g_things.getMarketThings()) or g_things.findThingTypeByAttr(ThingAttrMarket, 0) or {}
	for _, itemType in pairs(types) do
		...
		if item then
			local marketData = sanitizeMarketData(itemType)
			if marketData then
	...
	for id, name in pairs(g_game.getUnsortedCyclopediaItems() or {}) do
		local itemType = g_things.getThingType(id, ThingCategoryItem)
		local item = itemType and Item.create(itemType:getId())
		if item and marketItems[MarketCategory.Unassigned] then
			local marketData = table.copy(itemType:getMarketData() or {})
			marketData.category = MarketCategory.Unassigned
			marketData.name = name
			marketData.showAs = (marketData.showAs and marketData.showAs > 0) and marketData.showAs or itemType:getId()
			item:setId(marketData.showAs)
			table.insert(marketItems[MarketCategory.Unassigned], { displayItem = item, thingType = itemType, marketData = marketData })
		end
	end
	...
	if marketItems[MarketCategory.WeaponsAll] then
		table.sort(marketItems[MarketCategory.WeaponsAll], compareMarketItemsByNameCaseInsensitive)
	end
	...
	table.insert(categoryList, {MarketCategory.Gold, "Gold"})
...
-- Main Window
-- C++ sends (depotItems, offerCount, balance, vocation); old API had (offerCount, items)
function onMarketEnter(depotItems, offerCount, balance)
	configureList()
	depotLockerItems = depotItems
	...
	for _, pair in pairs(categoryList) do
		...
		widget.onClick = function()
			marketWindow.contentPanel.category:focusChild(widget)
			onSelectChildCategory(marketWindow.contentPanel.category, widget)
		end
	...
	show()
	marketWindow:focus()
	onUpdateResourceValue(nil, nil, balance)
	marketWindow.contentPanel.category.onChildFocusChange = function(self, selected) onSelectChildCategory(self, selected) end

	local firstCategory = marketWindow.contentPanel.category:getFirstChild()
	if firstCategory then
		firstCategory:focus()
		onSelectChildCategory(marketWindow.contentPanel.category, firstCategory)
	end
end

function onMarketBrowse(itemID, tier, buyList, sellList)
	itemID = itemID or getSelectedMarketItemId()
	tier = tier or getSelectedMarketItemTier()
	...
	for i, data in pairs(buyOffers) do
		...
		if widget.setIgnoreEqualFocus then
...
function onItemListValueChange(scroll, value, delta)
	...
	-- Normalmente offset 0 (itens inteiros a partir do topo; o ultimo slot fica parcial no rodape,
	-- preenchendo a area = SEM gap). No FIM da rolagem, desloca p/ cima pelo "overflow"
	-- (pool*36 - altura) p/ o ULTIMO item aparecer INTEIRO (o slot parcial vai pro topo).
	-- Resolve "ultimo item cortado pela metade" sem deixar espaco vazio.
	local list = marketWindow:recursiveGetChildById("itemList")
	local pool = #cache.SCROLL_MARKET_ITEMS.listPool
	local fullyVisible = math.floor(list:getHeight() / 36)
	local atBottom = #cache.SCROLL_MARKET_ITEMS.listData > fullyVisible
		and value >= (cache.SCROLL_MARKET_ITEMS.listMax - pool + 1)
	cache.SCROLL_MARKET_ITEMS.offset = atBottom and math.max(0, pool * 36 - list:getHeight()) or 0
	...
	for i, widget in ipairs(cache.SCROLL_MARKET_ITEMS.listPool) do
	  ...
	  if data and widget.item then
		...
		local itemTier = getMarketEntryTier(data)
		local isSelected = lastSelectedItem.itemId == data.thingType:getId() and (lastSelectedItem.tier or 0) == itemTier
		...
		local count = getDepotItemCount(data.thingType:getId(), itemTier)
		widget.item:setItemId(data.thingType:getId())
		applyRarityFrame(widget.item)

		updateMarketItemName(widget, data.marketData.name)

		widget.item:getItem():setCount(count)
		widget.item.itemIndex = index
		...
		setMarketItemTier(widget.item, itemTier)
		updateDepotCountLabel(widget, count)
		updateMarketItemBackground(widget, itemTier)
...
function onSelectChildCategory(widget, selected, keepFilter)
	if isRebuildingCategory or not selected then
		return true
	end
	...
	cache.SCROLL_MARKET_ITEMS.listFit = math.floor(itemList:getHeight() / 36) + 1 -- +1 preenche a area (sem gap); o offset no fim mostra o ultimo item inteiro
	...
	local selectedCategoryId = selected.categoryId or selected:getActionId()
	local clearHands = not lastSelectedCategory or not table.contains(enableCategories, selectedCategoryId)
	...
	-- Habilitar botoes
	if table.contains(enableCategories, selectedCategoryId) then
	...
	if not keepFilter then
		...
		if table.contains(enableClassification, selectedCategoryId) then
			marketWindow.contentPanel.classFilter:clearOptions()
	...
	applyRarityFrame(marketWindow.contentPanel.selectedItem)
	itemList.onChildFocusChange = function(self, selected, oldFocus) onSelectChildItem(self, selected, oldFocus) end

	local selectedCategoryItems = marketItems[selectedCategoryId] or {}
	for _, itemInfo in ipairs(selectedCategoryItems) do
		local itemTier = getMarketEntryTier(itemInfo)
		if checkSortMarketOptions(itemInfo) and not (showLockerOnly and getDepotItemCount(itemInfo.thingType:getId(), itemTier) == 0) then
	...
	for i, itemInfo in ipairs(cache.SCROLL_MARKET_ITEMS.listData) do
		...
		local itemTier = getMarketEntryTier(itemInfo)
		local count = getDepotItemCount(itemInfo.thingType:getId(), itemTier)
		...
		updateMarketItemName(widget, itemInfo.marketData.name)
	...
	if selectedCategoryId == 10 then
		marketWindow.contentPanel.mainMarket.getPotionsButton:setVisible(true)
	end

	isRebuildingCategory = false
	local firstChild = itemList:getFirstChild()
	if firstChild then
		itemList:focusChild(firstChild)
		onSelectChildItem(itemList, firstChild, nil)
	end
end
...
function onUpdateChildItem(itemID, tier)
	for _, widget in pairs(marketWindow.contentPanel.itemList:getChildren()) do
		if widget.item:getItem():getId() == itemID and widget.item:getItem():getTier() == tier then
			...
			if lastSelectedCategory then
				local itemIndex = widget.item.itemIndex or widget.item:getActionId()
				local itemInfo = cache.SCROLL_MARKET_ITEMS.listData[itemIndex]
				if itemInfo then
					widget.item:setTooltip(tr("%s%s%s%s", comma_value(count), "x", (count > 65000 and "+ " or " "), itemInfo.marketData.name))
				end
			end
...
function onSelectChildItem(widget, selected, oldFocus)
	if not selected or not selected.item or not selected.item:getItem() then return end
	...
	if not marketWindow.contentPanel.selectedItem then
		return true
	end

	marketWindow.contentPanel.selectedItem:setItemId(itemID)
	setMarketItemTier(marketWindow.contentPanel.selectedItem, itemTier)
	applyRarityFrame(marketWindow.contentPanel.selectedItem)
...
local function getMarketOfferFee(price, amount)
	local fee = math.ceil((price / 50) * amount)
	if fee < 20 then
		return 20
	end
	if fee > 1000000 then
...
local function ensureCreateAmount(minValue)
	local amount = mainMarket.amountCreateScrollBar:getValue()
	if amount < minValue then
		mainMarket.amountCreateScrollBar:setValue(minValue)
		amount = minValue
	end
	mainMarket.createOfferAmount:setText("Amount: " .. amount)
	return amount
end
...
function onPiecePriceEdit(widget)
	...
	local fee = getMarketOfferFee(numericValue, amount)
	mainMarket.createButton:setEnabled(false)
	...
	if currentActionType == 0 then
		...
		if isTibiaCoin then
			...
			if barCount > 0 then
				...
				amount = ensureCreateAmount(25)
			else
				mainMarket.amountCreateScrollBar:setRange(0, 0)
			end
		else
			if getTotalMoney() >= numericValue then
				barCount = math.min(maxCount, math.floor(getTotalMoney() / numericValue))
			end
			if barCount > 0 then
				mainMarket.amountCreateScrollBar:setRange(1, barCount)
				amount = ensureCreateAmount(1)
			else
				mainMarket.amountCreateScrollBar:setRange(0, 0)
			end
		end

		fee = getMarketOfferFee(numericValue, amount)
		...
		mainMarket.createButton:setEnabled(amount > 0 and balance >= grossProfit + fee)
		...
		if itemCount > 0 then
			if isTibiaCoin and itemCount < 25 then
				...
				mainMarket.amountCreateScrollBar:setRange(minAmount, math.min(maxCount, itemCount))
				amount = ensureCreateAmount(minAmount)
				mainMarket.createButton:setEnabled(amount > 0)
			end
		else
			mainMarket.amountCreateScrollBar:setRange(0, 0)
		end
...
function createMarketOffer()
	...
	local amount = tonumber(n:gsub("%D", "")) or 0
	local price = tonumber(mainMarket.grossAmount.value)
	if amount <= 0 or not price or price <= 0 then
		return
	end
	if currentActionType == 0 and getTotalMoney() < (price * amount) + getMarketOfferFee(price, amount) then
		return
	end
	mainMarket.amountCreateScrollBar:setRange(0, 0)
	mainMarket.createButton:setEnabled(false)
	...
	local anonymous = mainMarket.anonymous:isChecked() and 1 or 0
	g_game.sendMarketCreateOffer(currentActionType, lastSelectedItem.itemId, lastSelectedItem.tier or 0, amount, price, anonymous)
end
	...
	for c = MarketCategory.First, MarketCategory.Last do
		...
		if marketItem then
			for _, data in pairs(marketItem) do
				local itemTier = getMarketEntryTier(data)
				if not checkSortMarketOptions(data) or (showLockerOnly and getDepotItemCount(data.thingType:getId(), itemTier) == 0)then
					goto continue
				end
...
function checkSortMarketOptions(itemData)
	...
	if sortButtons["vocButton"] then
		...
		if type(itemVocation) == "number" then
			if itemVocation > 0 then
				local vocBitMask = Bit.bit(playerVocation)
				if not Bit.hasBit(itemVocation, vocBitMask) then
					return false
				end
			end
		elseif type(itemVocation) == "table" then
...
function onSortMarketFields(widget, checked)
	...
	elseif table.contains({'classFilter', 'tierFilter'}, widget:getId()) then
		...
		elseif widget:getId() == "tierFilter" then
			sortButtons["tierFilter"] = checked > 0 and (checked - 1) or 0
		end
...
function onMarketDetail(itemID, tier, details, purchase, sale)
	...
	for key, value in pairs(details or {}) do
		if type(key) == "number" and type(value) == "string" and #value > 0 then
			table.insert(detailKeys, key)
		end
	end
	table.sort(detailKeys)

	for _, i in ipairs(detailKeys) do
		local str = details[i]
...
-- Adapters: C++ sends arrays; these functions convert to the named-field format expected above.

local onMarketBrowseImpl = onMarketBrowse
function onMarketBrowse(itemOrOffers, tierOrNames, offersOrNil, namesOrNil)
	local itemID = getSelectedMarketItemId()
	local tier = getSelectedMarketItemTier()
	local intOffers, nameOffers
	local requestVar = nil

	if type(itemOrOffers) == "number" then
		itemID = itemOrOffers
		requestVar = itemOrOffers
		tier = tierOrNames or 0
		intOffers = offersOrNil or {}
		nameOffers = namesOrNil or {}
	else
		intOffers = itemOrOffers or {}
		nameOffers = tierOrNames or {}
	end

	if not intOffers then return end
	local function makeOffer(intOffer, playerName)
		return {
			action = intOffer[1], amount = intOffer[2], counter = intOffer[3],
			itemId = intOffer[4], price = intOffer[5], state = intOffer[6],
			timestamp = intOffer[7], var = intOffer[8], itemTier = intOffer[9],
			tier = intOffer[9] or 0,
			holder = playerName or "",
			playerName = playerName or ""
		}
	end

	local var = (#intOffers > 0) and intOffers[1][8] or (requestVar or 0)
	local buyList, sellList = {}, {}
	itemID = (#intOffers > 0 and intOffers[1][4]) or itemID
	local firstOfferTier = (#intOffers > 0 and tonumber(intOffers[1][9])) or 0
	tier = firstOfferTier > 0 and firstOfferTier or tier

	for i, intOffer in ipairs(intOffers) do
		local offer = makeOffer(intOffer, nameOffers and nameOffers[i])
		itemID = intOffer[4] or itemID
		local offerTier = tonumber(intOffer[9]) or 0
		tier = offerTier > 0 and offerTier or tier
		if intOffer[1] == 0 then
			table.insert(buyList, offer)
		else
			table.insert(sellList, offer)
		end
	end
	...
	elseif var == 2 then
		...
		onMarketBrowseImpl(itemID, tier, buyList, sellList)
	end
end

local onMarketDetailImpl = onMarketDetail
function onMarketDetail(itemId, descriptions, purchaseStats, saleStats, itemTier)
	local function convertStats(stats)
		local result = {}
		for _, s in ipairs(stats or {}) do
			table.insert(result, {
				numTransactions = s[3], totalPrice = s[4],
				highestPrice = s[5], lowestPrice = s[6]
			})
		end
		return result
	end
	onMarketDetailImpl(itemId, itemTier or 0, descriptions, convertStats(purchaseStats), convertStats(saleStats))
end
