MarketOwnOffers = {
	...
	selectedSellCounter = {counter = 0, action = 0},
	selectedBuyCounter = {counter = 0, action = 0}
}
...
function MarketOwnOffers.onParseMyOffers(buyOffers, sellOffers)
	...
	sellScrollbar.onValueChange = function(self, value, delta) MarketOwnOffers.onTopListValueChange(self, value, delta) end
	...
	buyScrollbar.onValueChange = function(self, value, delta) MarketOwnOffers.onBottomListValueChange(self, value, delta) end

	window.sellOffersList.onChildFocusChange = function(self, selected) MarketOwnOffers.onSelectMyOffersChild(self, selected, true) end
	window.buyOffersList.onChildFocusChange = function(self, selected) MarketOwnOffers.onSelectMyOffersChild(self, selected, false) end
...
function MarketOwnOffers.cancelMarketOffer(selling)
	...
	local targetAction = widget and widget:getActionId() or (selling and MarketOwnOffers.selectedSellCounter.action or MarketOwnOffers.selectedBuyCounter.action)
	local targetOffer = targetList[targetAction]
	local selectedCounter = widget and widget.counter or (selling and MarketOwnOffers.selectedSellCounter.counter or MarketOwnOffers.selectedBuyCounter.counter)
	if (not targetOffer or targetOffer.counter ~= selectedCounter) and selectedCounter and selectedCounter > 0 then
		for _, offer in pairs(targetList) do
			if offer.counter == selectedCounter then
				targetOffer = offer
				break
			end
		end
	end
	local timestamp = targetOffer and targetOffer.timestamp or (widget and widget.timestamp)
	local counter = targetOffer and targetOffer.counter or (widget and widget.counter)
	...
	g_game.sendMarketCancelOffer(timestamp, counter)
end
