...
function WheelOfDestiny.onMouseMove(widget, position, offset)
  ...
  bar:setImageSource("/mods/game_cyclopedia/images/ui/mosnter-bar")
  ...
  if WheelOfDestiny.pointInvested[index] > 0 then
    wheelOfDestinyWindow.info.tabContent.information.tabContent.dedication2:setColor("$var-text-cip-color")
  else
    wheelOfDestinyWindow.info.tabContent.information.tabContent.dedication2:setColor("$var-cip-inactive-color")
  end
  ...
  if type(conviction) == "string" then
    ...
    if WheelOfDestiny.pointInvested[index] >= bonus.maxPoints then
      wheelOfDestinyWindow.info.tabContent.information.tabContent.conviction2:setColor("$var-text-cip-color")
    else
      ...
      if bonus and table.contains(VesselIndex[bonus.domain - 1], index - 1) then
        ...
        if gem then
          ...
          if #enabled == 0 and gem.lesserBonus > -1 then
            widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-basicmods")
          ...
          elseif #enabled == 2 and gem.supremeBonus > -1 then
            widget:setImageSource("/images/game/destiny_wheel/icons-skillwheel-suprememods")
...
function WheelOfDestiny.onDestinyWheel(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested, usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
  ...
  if vocationId == 1 then
    wheelPanel.vocationWheel:setImageSource('/images/game/destiny_wheel/wheel-vocations/backdrop_skillwheel_knight')
  ...
  elseif vocationId == 2 then
    wheelPanel.vocationWheel:setImageSource('/images/game/destiny_wheel/wheel-vocations/backdrop_skillwheel_paladin')
    wheelPanel:recursiveGetChildById("perkIconTopLeft"):setImageClip("0 0 34 34")
  ...
  elseif vocationId == 3 then
    wheelPanel.vocationWheel:setImageSource('/images/game/destiny_wheel/wheel-vocations/backdrop_skillwheel_sorcerer')
    wheelPanel:recursiveGetChildById("perkIconTopLeft"):setImageClip("0 0 34 34")
  ...
  elseif vocationId == 4 then
    wheelPanel.vocationWheel:setImageSource('/images/game/destiny_wheel/wheel-vocations/backdrop_skillwheel_druid')
    wheelPanel:recursiveGetChildById("perkIconTopLeft"):setImageClip("0 0 34 34")
  ...
  elseif vocationId == 5 then
    wheelPanel.vocationWheel:setImageSource('/images/game/destiny_wheel/wheel-vocations/backdrop_skillwheel_monk')
    wheelPanel:recursiveGetChildById("perkIconTopLeft"):setImageClip("0 0 34 34")
...
function WheelOfDestiny.configureDedication(index)
  ...
  if WheelOfDestiny.pointInvested[index] > 0 then
    wheelOfDestinyWindow.selection.tabContent.dedication:setColor("$var-text-cip-color")
  else
    wheelOfDestinyWindow.selection.tabContent.dedication:setColor("$var-cip-inactive-color")
  end
end
...
function WheelOfDestiny.configureConviction(index)
  ...
  if type(conviction) == "string" then
    ...
    if WheelOfDestiny.pointInvested[index] >= bonus.maxPoints then
      wheelOfDestinyWindow.selection.tabContent.conviction:setColor("$var-text-cip-color")
    else
...
function WheelOfDestiny.configureVessels()
  wheelOfDestinyWindow.vessels.tabContent:destroyChildren()
  wheelOfDestinyWindow.vessels.tabContentScroll:setVisible(false)

  local bonus = getVesselBonus()
  for _, data in pairs(bonus) do
    local widget = g_ui.createWidget("PerksPanel", wheelOfDestinyWindow.vessels.tabContent)
    ...
    if wheelOfDestinyWindow.vessels.tabContentScroll:getMaximum() > 0 then
      wheelOfDestinyWindow.vessels.tabContentScroll:setVisible(true)
    end
  end
end
...
function WheelOfDestiny.create(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested, usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
  ...
  local function incrementBonusCount(bonus, bonusType)
    if bonus ~= -1 then
      local data = bonusType[tostring(bonus)]
      if data then
        bonusType[tostring(bonus)] = data + 1
      else
        bonusType[tostring(bonus)] = 1
      end
    end
  end
...
function WheelOfDestiny.configurePassives()
  ...
  for domain, points in ipairs(WheelOfDestiny.passivePoints) do
    ...
    if points < 250 then
      widget:setImageSource("/images/game/destiny_wheel/backdrop_skillwheel_largebonus_front0_"..passivel)
      widgetPercent:setPercent(math.floor(((points - 0) / (250 - 0)) * 100))
    elseif points < 500 then
      widget:setImageSource("/images/game/destiny_wheel/backdrop_skillwheel_largebonus_front1_"..passivel)
      widgetPercent:setPercent(math.floor(((points - 250) / (500 - 250)) * 100))
    elseif points < 1000 then
      widget:setImageSource("/images/game/destiny_wheel/backdrop_skillwheel_largebonus_front2_"..passivel)
      widgetPercent:setPercent(math.floor(((points - 500) / (1000 - 500)) * 100))
    else
      widget:setImageSource("/images/game/destiny_wheel/backdrop_skillwheel_largebonus_front3_"..passivel)
...
function WheelOfDestiny.showNewPreset(createPreset)
  ...
  g_client.setInputLockWidget(newPresetWindow)
...
function WheelOfDestiny.onRenamePreset()
  ...
  g_client.setInputLockWidget(renamePresetWindow)
end
...
function WheelOfDestiny.configureEquippedGems()
  for i = 0, 3 do
    ...
    background:setImageSource("/images/game/destiny_wheel/" .. backGroundImage)
    ...
    if data then
      ...
      socket:setImageSource("/images/game/destiny_wheel/icons-skillwheel-sockets")
      socket:setVisible(true)
      if not showSocket then
        socket:setImageClip("0 0 34 34")
      end
    else
      socket:setImageSource("/images/game/destiny_wheel/backdrop_skillwheel_socket_inactive.png")
      socket:setImageClip("0 0 34 34")
    end
  end
end
