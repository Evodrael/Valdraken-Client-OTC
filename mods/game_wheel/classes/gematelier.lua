...
local function getGemListIndex(widget)
	if not widget then
		return nil
	end

	if widget.gemListIndex then
		return widget.gemListIndex
	end

	local index = tonumber(widget:getActionId())
...
local function getGemDataFromWidget(widget)
	if widget and widget.cache then
		return widget.cache, getGemListIndex(widget)
	end

	local index = getGemListIndex(widget)
	if not index then
		return nil, nil
	end

	return currentGemList[index], index
end

local function clearGemSelection()
	...
	if panel.clickedContent then
		panel.clickedContent:setVisible(false)
	end
	if panel.cleanContent then
...
function GemAtelier.redirectToGem(gemData)
	...
	for i, data in pairs(totalGemList) do
		...
		widget.cache = data
		widget.gemListIndex = #currentGemList
		widget:setActionId(widget.gemListIndex)
		gemCount = gemCount + 1
...
function GemAtelier.showGems(selectFirst, lastIndex)
	...
	totalGemList = {}
	currentGemList = {}
	local selectedIndex = tonumber(lastIndex) or getGemListIndex(lastSelectedGem)
	lastSelectedGem = nil
	for i, data in pairs(WheelOfDestiny.atelierGems) do
	...
	if #children == 0 then
		...
		if selectFirst then
			focusWidget = gemList:getFirstChild()
		elseif selectedIndex and children[selectedIndex] then
			focusWidget = children[selectedIndex]
		end

		if focusWidget then
			gemList:focusChild(focusWidget)
			GemAtelier.onSelectGem(focusWidget, true)
		else
			focusWidget = gemList:getFirstChild()
			gemList:focusChild(focusWidget)
			GemAtelier.onSelectGem(focusWidget, true)
...
function GemAtelier.showGemRevelation()
	...
	local totalBalance = player:getResourceValue(ResourceBank) + player:getResourceValue(ResourceInventary)
	local resources = {
		[0] = player:getResourceValue(ResourceLesserGem),
		[1] = player:getResourceValue(ResourceRegularGem),
		[2] = player:getResourceValue(ResourceGreaterGem)
	}
	...
	for i = 0, 2 do
		...
		if not gemInfo:isTextWraped() then
		...
		setStringColor(gemString, comma_value(GemRevealPrice[i] / 1000) .. " k", textColor)
		revealCost.gold:setText(comma_value(GemRevealPrice[i] / 1000) .. " k")
		revealCost.gold:setColor(textColor)
...
function GemAtelier.onSelectGem(selected, clicked)
	...
	local gemData = getGemDataFromWidget(selected)
	if not gemData then
		local gemList = gemAtelierWindow and gemAtelierWindow:recursiveGetChildById("gemContent")
		local firstGem = gemList and gemList:getFirstChild()
		if firstGem and firstGem ~= selected then
			gemList:focusChild(firstGem)
			GemAtelier.onSelectGem(firstGem, true)
			return true
		end
  ...
  GemAtelier.createGemInformation(widgetMods.modLabel0, gemData.lesserBonus, false, false, gemData, 0)
	...
	local switchCost = GemSwitchPrice[gemData.gemType] or 0
	local switchCostText = comma_value(switchCost / 1000) .. " k"
	local switchCostColor = totalBalance >= switchCost and "#c0c0c0" or "#d33c3c"

	panel.clickedContent.switchCost.gold:setText(switchCostText)
	panel.clickedContent.switchCost.gold:setColor(switchCostColor)
	panel.clickedContent.switchCost:setTooltip(comma_value(switchCost))
...
function GemAtelier.manageVessel(remove)
	...
	local gemData, selectedIndex = getGemDataFromWidget(lastSelectedGem)
	if not gemData then
		clearGemSelection()
		return true
	end
	...
	GemAtelier.showGems(false, selectedIndex)
end
...
function GemAtelier.onSwitchDomain(button)
	...
	local gemData = getGemDataFromWidget(lastSelectedGem)
	if not gemData then
		clearGemSelection()
		return true
	end

	g_game.sendGemAtelierAction(2, gemData.gemID)
end
...
function GemAtelier.onLockGem(button)
	...
	g_game.sendGemAtelierAction(3, gemData.gemID)
end
...
function GemAtelier.onHoverGem(self, hovered)
	...
	if hovered and not self.pointerCursorPushed then
		self.pointerCursorPushed = true
		g_mouse.pushCursor('pointer')
	elseif not hovered and self.pointerCursorPushed then
		self.pointerCursorPushed = nil
