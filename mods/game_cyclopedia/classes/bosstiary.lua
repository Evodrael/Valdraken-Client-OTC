...
local sortTypes = {
	[1] = {name = "Bane", icon = "/mods/game_cyclopedia/images/icons/icon-bosstiary-1"},
	[2] = {name = "Archfoe", icon = "/mods/game_cyclopedia/images/icons/icon-bosstiary-2"},
	[3] = {name = "Nemesis", icon = "/mods/game_cyclopedia/images/icons/icon-bosstiary-3"},
	[4] = {name = ""}, -- mmm
	[5] = {name = ""},	-- mmm
	[6] = {name = "No Kills"},
	[7] = {name = "Few Kills"},
...
local function normalizeBossEntry(data)
	...
	local bossId = data[1] or data.raceId or data.bossId
	if not bossId or bossId == 0 then
		return nil
	end

	data[1] = bossId
	data[2] = data[2] or data.category or 0
	data[3] = data[3] or data.kills or 0
	data[4] = data[4] or data.isTrackerActived or data.isTracked or 0
	return data
end
...
function Bosstiary.configureBossList(data, text)
	...
	for _, entry in pairs(data or {}) do
		local v = normalizeBossEntry(entry)
		if v then
			v[5] = monsterList[v[1]]
			normalizedData[#normalizedData + 1] = v
		end
	end

	table.sort(normalizedData, function(a, b)
		local aKills = a[3] or 0
		local bKills = b[3] or 0
		...
		local aName = a[5] and a[5][1] or 0
		local bName = b[5] and b[5][1] or 0
		return aName < bName
	end)

	local tmpPage = 1
	local pageEntries = 0
	for _, v in pairs(normalizedData) do
		local _bossID = v[1]
		local _category = v[2]
		local _kills = v[3]
...
function Bosstiary.initSortFields()
  ...
  if not inforBox then
	...
	for k, data in ipairs(sortTypes) do
		...
		if infor.checkMarks.setChangeCursorImage then
  ...
  if not bosstiaryMonsterPanel then
	...
	for _, data in pairs(bosstiaryCreatures[bosstiaryCurrentPage]) do
		...
		if data.outfit then
			...
			monsters.monster.outfit:setOutfit({type = data.outfit[2], auxType = data.outfit[3], head = data.outfit[4], body = data.outfit[5], legs = data.outfit[6], feet = data.outfit[7], addons = data.outfit[8], shaders = monsterShader})
		...
		if currentKills >= baseKill.firstUnlock then
			monsters.starFisrt:setImageSource("/mods/game_cyclopedia/images/icons/star/enable1")
		...
		if currentKills >= baseKill.secondUnlock then
			monsters.starSecond:setImageSource("/mods/game_cyclopedia/images/icons/star/enable2")
		...
		if currentKills >= baseKill.thirdUnlock then
			monsters.progressFirst.first:setImageSource('/mods/game_cyclopedia/images/ui/monster-bar-green')
			monsters.progressSecond.second:setImageSource('/mods/game_cyclopedia/images/ui/monster-bar-green')
			monsters.progressThird.third:setImageSource('/mods/game_cyclopedia/images/ui/monster-bar-green')
			monsters.starThird:setImageSource("/mods/game_cyclopedia/images/icons/star/enable3")
		end

		local category = data.category + 1
		monsters.monster.outfit:setStaticWalking(true)
		monsters.progressSecond.second.killCounterLabel:setText(data.kills)
		monsters.iconBosstiary:setImageSource('/mods/game_cyclopedia/images/icons/icon-bosstiary-' .. data.category + 1)
