...
MapCyclopedia.boostedAreas = {}
MapCyclopedia.boostPrice = 0

MapCyclopedia.setup = function()
    MapCyclopedia.currentArea = 0
    MapCyclopedia.currentAreaName = ''
    MapCyclopedia.boostedAreas = g_game.getBoostedAreas and g_game.getBoostedAreas() or {}
    MapCyclopedia.boostPrice = g_game.getMapBoostPrice and g_game.getMapBoostPrice() or 0
    g_game.requestResource(ResourceBank)
    g_game.requestResource(ResourceInventary)
    -- DESATIVADO temporariamente: o servidor TFS nao trata o extended opcode 205
    -- (Cyclopedia Map), entao o cliente logava "extended opcodes are not enabled".
    -- Reativar quando o handler 205 existir no servidor.
    -- if g_game.requestCyclopediaMapData then
    --     g_game.requestCyclopediaMapData()
    -- end
    ...
    if minimap then
        ...
        RealMap.setUIMarkers(minimap, playerPosition, 120)
        minimap:clearWaypoints()
        minimap:clearRoutePath()
        RealMap.setCameraPosition(minimap, playerPosition)
        RealMap.setCrossPosition(minimap, playerPosition)
        ...
        local discoveringButton = VisibleCyclopediaPanel:recursiveGetChildById('discoveringButton')
        ...
        if discoveringButton then
            discoveringButton.onClick = MapCyclopedia.onDiscoveringClick
        end
    end
end

function MapCyclopedia.onCyclopediaMapData(boostedAreas, boostPrice, currentArea)
    MapCyclopedia.boostedAreas = boostedAreas or {}
    MapCyclopedia.boostPrice = tonumber(boostPrice) or 0

    if currentArea and currentArea > 0 then
        MapCyclopedia.currentArea = currentArea
    end

    if VisibleCyclopediaPanel and MapCyclopedia.currentArea > 0 then
        MapCyclopedia.setImprovevedValue(MapCyclopedia.currentArea)
    end
end
...
function MapCyclopedia.onChangeButtonMarks(button, i)
    ...
    local minimap = VisibleCyclopediaPanel:recursiveGetChildById('minimap')
    if minimap and minimap.ignoreWidget then
    ...
    if isChecked then
        ...
        if minimap and minimap.unignoreWidget then
...
function MapCyclopedia.setImprovevedValue(areaId)
    ...
    if g_things.getAreaById then
        areaId = g_things.getAreaById(areaId)
    end
    if areaId == 0 then
        return
    end
    ...
    local maps = next(MapCyclopedia.boostedAreas or {}) and MapCyclopedia.boostedAreas or (g_game.getBoostedAreas and g_game.getBoostedAreas() or {})
    local improved = maps[areaId] or {second = 0}
    local boostPrice = MapCyclopedia.boostPrice > 0 and MapCyclopedia.boostPrice or (g_game.getMapBoostPrice and g_game.getMapBoostPrice() or 0)
    ...
    if progressBox then
        progressBox:setValue(improved.second, 0, boostPrice > 0 and boostPrice or 1)
        progressBox:setTooltip(string.format("%s of %s gold donated -\nYour game world needs to donate at least %s gold\nto be able to win the Improved Respawn Rate for this area.", comma_value(improved.second), comma_value(boostPrice), comma_value(boostPrice)))
    end

    MapCyclopedia.currentArea = areaId
end

function MapCyclopedia.onDiscoveringClick()
    if MapCyclopedia.currentArea == 0 or not g_game.startDiscoveringMap then
        return
    end

    -- DESATIVADO: extended opcode 205 nao tratado pelo servidor TFS.
    -- g_game.startDiscoveringMap(MapCyclopedia.currentArea)
end
...
function MapCyclopedia.onDonateClick()
    ...
    local yesCallback = function()
        -- DESATIVADO: extended opcode 205 nao tratado pelo servidor TFS.
        -- if g_game.doDonateMap then
        --     g_game.doDonateMap(MapCyclopedia.currentArea, donateValue)
        -- end
        noCallback()
    end
