...
local function normalizeInspectionDescription(data)
	local detail = data.detail or data[1]
	local description = data.description or data[2]

	if inspectionLabelNames[tostring(description or "")] and not inspectionLabelNames[tostring(detail or "")] then
		detail, description = description, detail
	end
...
local function ensureWindowPanel()
	if windowPanel and windowPanel:getParent() then
		return true
	end
...
function Character.initMainWindow()
	if not inspectPlayer.playerData or not ensureWindowPanel() then
		return
	end
	...
	-- Remove o nome do world (titleLabel) que aparecia junto do outfit, abaixo do nome do jogador.
	VisibleCyclopediaPanel.outfitWindow.titleLabel:setText("")
	VisibleCyclopediaPanel.outfitWindow.titleLabel:setVisible(false)
	...
	for _, data in pairs(inspectPlayer.playerData) do
		if inspectionLabelNames[tostring(data.detail or "")] then
	...
	-- create buttons
	local buttonNames = {"General Stats","Battle Results","Achievements","Item Summary","Appearances","Character Titles","Store Summary"}
	for k, v in pairs(buttonNames) do
		...
		SlotPanel.buttons:setIcon("/mods/game_cyclopedia/images/ui/icon/icon-character-"..k)
...
function Character.onChangeCharacterPanel(buttonId)
	...
	local button7 = g_ui.getRootWidget():recursiveGetChildById('button7')
	local buttons = {button1, button2, button3, button4, button5, button6, button7}
	...
	for i, btn in ipairs(buttons) do
		...
		button1.ButtonExtraPanel:setVisible(false)
		button2.ButtonExtraPanel:setVisible(false)
	...
	if buttonId == 1 then
		...
		button1.ButtonExtraPanel:setVisible(true)
		...
		for i = 1, #widgets do
			...
			button:setIcon("/mods/game_cyclopedia/images/ui/icon/icon-character-".. widgets[i].icon)
		end
	end
	...
	if buttonId == 2 then
		...
		button2.ButtonExtraPanel:setVisible(true)
		...
		button2.ButtonExtraPanel.buttonsExtra1:setIcon("/mods/game_cyclopedia/images/ui/icon/icon-character-"..21)
		button2.ButtonExtraPanel.buttonsExtra2:setIcon("/mods/game_cyclopedia/images/ui/icon/icon-character-"..22)
	...
	if buttonId >= 3 and buttonId <= 7 then
		buttons[buttonId].buttons:setOn(true)
		buttons[buttonId].buttons.arrowRef:setImageSource("/mods/game_cyclopedia/images/ui/arrow-right")
		...
		elseif buttonId == 6 then windowPanel:destroyChildren() Titles.initPanel() g_game.requestCyclopediaData(11)
		elseif buttonId == 7 then windowPanel:destroyChildren() Character.initStoreSummary() g_game.requestCyclopediaData(8) end
	end
...
function Character.toggleInspectCharacter(widget)
	...
	for _, str in pairs(inspectPlayer.playerData) do
		if inspectionLabelNames[tostring(str.detail or "")] then
			local widget = g_ui.createWidget('InspectLabel', windowPanel.itemInfor)
...
function Character.showStats()
	windowPanel:setImageSource("/mods/game_cyclopedia/images/ui/panel-background")
...
local function skillData(total, equipment, imbuement, wheel, event, flat, concoction, amplification)
	return {
		skillPercent = percentValue(total),
		fromEquipment = percentValue(equipment),
		fromImbuement = percentValue(imbuement),
		fromSkillWheel = percentValue(wheel),
		fromEvent = percentValue(event),
		fromFlatBonus = percentValue(flat),
		fromConcoction = percentValue(concoction),
		fromAmplification = percentValue(amplification)
	}
end

local function elementMap(list)
	local result = {}
	for _, entry in pairs(list or {}) do
		local element = entry.element or entry[1]
		local value = entry.value or entry[2]
		if element then
			result[element] = percentValue(value)
		end
	end
	return result
end

local function skillBonusMap(list)
	local result = {}
	for _, entry in pairs(list or {}) do
		local skillId = entry.skillId or entry[1]
		if skillId then
			result[skillId] = {
				first = percentValue(entry.valueA or entry[2]),
				second = percentValue(entry.valueB or entry[3])
			}
		end
	end
	return result
...
function Character.onCyclopediaOffenceStatsData(data)
	if not data then
		return true
	end

	local leechData = {
		skillData(data.lifeLeechTotal, data.lifeLeechEquipament, data.lifeLeechImbuement, data.lifeLeechWheel, data.lifeLeechEventBonus),
		skillData(data.manaLeechTotal, data.manaLeechEquipament, data.manaLeechImbuement, data.manaLeechWheel, data.manaLeechEventBonus),
		skillData(data.critChanceTotal, data.critChanceEquipament, data.critChanceImbuement, data.critChanceWheel, 0, data.critChanceFlat, data.critChanceConcoction),
		skillData(data.critDamageTotal, data.critDamageEquipament, data.critDamageImbuement, data.critDamageWheel, 0, data.critDamageFlat, data.critDamageConcoction)
	}

	local perfectShotData = {}
	for i, value in pairs(data.perfectShotDamage or {}) do
		perfectShotData[i] = value
	end

	local distanceFactor = {}
	for _, accuracy in pairs(data.weaponAccuracy or {}) do
		distanceFactor[accuracy.range or accuracy[1] or 1] = percentValue(accuracy.chance or accuracy[2])
	end

	local targetDamage = {}
	for _, target in pairs(data.damageSpecificTargets or {}) do
		targetDamage[target.name or target[1] or ""] = percentValue(target.value or target[2])
	end

	Character.onCyclopediaOffence(
		leechData,
		percentValue(data.cleavePercent),
		perfectShotData,
		data.flatDamage or 0,
		data.flatDamageBase or 0,
		data.flatDamageWheel or 0,
		{
			value = data.weaponAttack or 0,
			valueElement = data.weaponElement or 0,
			valueFlat = data.weaponFlatModifier or 0,
			valueEquipment = data.weaponDamage or 0,
			valueFromSkill = data.weaponSkillModifier or 0,
			valueSkill = data.weaponSkillType or 0,
			valueMastery = 0,
			valueConverted = percentValue(data.weaponElementDamage),
			valueConvertedElement = data.weaponElementType or 0
		},
		distanceFactor,
		{ powerfulFoeDamage = percentValue(data.damagePowerfulFoes), bestiaryDamage = targetDamage },
		{ runeCritical = percentValue(data.offensiveRuneDamage), meleeCritical = percentValue(data.autoAttackDamage), elementMap = elementMap(data.damageElements) },
		{ runeCritical = percentValue(data.critDamageOffensiveRunes), meleeCritical = percentValue(data.critDamageAutoAttack), elementMap = elementMap(data.critDamageElements) },
		{ manaOnHit = data.manaGainHit or 0, manaOnKill = data.manaGainKill or 0, healthOnHit = data.lifeGainHit or 0, healthOnKill = data.lifeGainKill or 0 },
		skillBonusMap(data.extraDamageSkills),
		skillBonusMap(data.extraDamageSpells),
		skillBonusMap(data.extraHealingSpells)
	)
end

function Character.onCyclopediaDefenceStatsData(data)
	if not data then
		return true
	end

	local elementalProtections = {}
	for _, resistance in pairs(data.resistances or {}) do
		local element = resistance.element or resistance[1]
		if element then
			elementalProtections[element + 1] = percentValue(resistance.value or resistance[2])
		end
	end

	Character.onCyclopediaDefence(
		skillData(data.dodgeTotal, data.dodgeBase, 0, data.dodgeWheel, data.dodgeBonus, 0, 0, 0),
		data.magicShieldCapacity or 0,
		data.magicShieldCapacityFlat or 0,
		percentValue(data.magicShieldCapacityPercent),
		data.reflectPhysical or 0,
		data.armor or 0,
		{
			value = data.defense or 0,
			valueEquipment = data.defenseEquipment or 0,
			valueFromSkill = data.shieldingSkill or 0,
			valueSkill = data.defenseSkillType or 0,
			valueMastery = data.defenseWheel or 0,
			valueCombatTatcis = 0
		},
		{
			skillPercent = percentValue(data.mitigation),
			fromDefense = percentValue(data.mitigationBase),
			fromShielding = percentValue(data.mitigationShield),
			fromEquipment = percentValue(data.mitigationEquipment),
			fromSkillWheel = percentValue(data.mitigationWheel),
			fromCombatTatics = percentValue(data.mitigationCombatTactics)
		},
...
function Character.onCyclopediaMiscStatsData(data)
	if not data then
		return true
	end

	local concoctions = {}
	for _, entry in pairs(data.concoctions or {}) do
		local id = entry.id or entry[1]
		if id then
			concoctions[id] = entry.duration or entry[2] or 0
		end
	end

	local cooldowns = {}
	for _, entry in pairs(data.activeFoods or {}) do
		local id = entry.id or entry[1]
		if id then
			cooldowns[id] = entry.duration or entry[2] or 0
		end
	end

	Character.onCyclopediaMisc(
		skillData(data.momentumTotal, data.momentumBase, 0, data.momentumWheel, data.momentumBonus),
		skillData(data.dodgeTotal, data.dodgeBase, 0, data.dodgeWheel, data.dodgeBonus),
		skillData(data.damageReflectionTotal, data.damageReflectionBase, 0, 0, data.damageReflectionBonus),
		data.haveBlesses or 0,
		data.totalBlesses or 0,
		concoctions,
		cooldowns
	)
end
...
function Character.onCyclopediaOffence(data, cleavePercent, perfectShotData, damageAndHealing, damageAndHealingLevel, damageAndHealingWheel, attackData, distanceFactor,
	...
	updatePanel(
		...
		{
			attackValue = { text = attackData.value, visible = true, icon = '/mods/game_cyclopedia/images/icons/stats/element_' .. attackData.valueElement },
			...
			attackSkill = { text = attackData.valueFromSkill .. " from " .. (SkillNames[attackData.valueSkill] or "") .. " Fighting", visible = attackData.valueFromSkill ~= 0 },
			attackCombatTatics = { text = attackData.valueMastery .. " from Combat Tatics", visible = attackData.valueMastery ~= 0 },
			convertedDamage = { text = "+" .. attackData.valueConverted .. "%", visible = attackData.valueConverted ~= 0, icon = '/mods/game_cyclopedia/images/icons/stats/element_' .. attackData.valueConvertedElement }
		}
	)
...
function Character.onCyclopediaDefence(dodgeData, shieldCapacity, shieldDirect, shieldPercentage, damageReflect, armorValueData, defenseData, mitigationData, elementalProtections, mantraDefense)
	...
	updatePanel(
		Defensestats:recursiveGetChildById("infoPanel0"),
		{
			...
			defenceSkill = { text = defenseData.valueFromSkill .. " from " .. (SkillNames[defenseData.valueSkill] or ""), visible = defenseData.valueFromSkill ~= 0 },
...
-- Store Summary (subtipo 8, opcode 0xDA). O servidor envia o pacote ao chamar
-- g_game.requestCyclopediaData(8); o parser C++ dispara onParseCyclopediaStoreSummary.
function Character.initStoreSummary()
  windowPanel = VisibleCyclopediaPanel:recursiveGetChildById("windowPanel")
  StoreSummary = g_ui.createWidget('StoreSummary', windowPanel)
  StoreSummary:setId("StoreSummary")
  windowPanel:setImageSource("")
end

local function storeSummaryAddSection(list, title)
  local w = g_ui.createWidget('StoreSummarySection', list)
  w:setText(title)
  return w
end

local function storeSummaryAddLabel(list, text, value)
  local w = g_ui.createWidget('StoreSummaryLabel', list)
  w:setText(text)
  if w.value then
    w.value:setText(value ~= nil and tostring(value) or "")
  end
  return w
end

-- Formata segundos restantes de XP boost em "Xh Ym" (ou "None").
...
function Character.onParseCyclopediaStoreSummary(xpBoostTime, dailyRewardXpBoostTime, blessings,
    preySlotsUnlocked, preyWildcards, hasPermanentWeeklyTaskExpansion, instantRewards,
    hasCharmExpansion, hirelingsObtained, hirelingSkills, houseItems)
  -- A janela pode não estar montada (o usuário trocou de aba antes do pacote chegar).
  if not StoreSummary or StoreSummary:isDestroyed() then
    return
  end
  local list = StoreSummary:recursiveGetChildById('list')
  if not list then return end
  list:destroyChildren()

  -- XP Boosts
  storeSummaryAddSection(list, tr("XP Boosts"))
  storeSummaryAddLabel(list, tr("Store XP Boost Time"), formatBoostTime(xpBoostTime))
  storeSummaryAddLabel(list, tr("Daily Reward XP Boost Time"), formatBoostTime(dailyRewardXpBoostTime))

  -- Blessings
  storeSummaryAddSection(list, tr("Blessings"))
  for _, b in ipairs(blessings or {}) do
    -- cada entrada = { nome, obtido(0/1) }
    local name = b[1] or b.name or "?"
    local obtained = (b[2] or b.obtained or 0)
    storeSummaryAddLabel(list, name, (tonumber(obtained) == 1) and tr("Yes") or tr("No"))
  end

  -- Prey / Tasks
  storeSummaryAddSection(list, tr("Prey & Tasks"))
  storeSummaryAddLabel(list, tr("Permanent Prey Slots"), preySlotsUnlocked or 0)
  storeSummaryAddLabel(list, tr("Prey Wildcards"), preyWildcards or 0)
  if hasPermanentWeeklyTaskExpansion ~= nil then
    storeSummaryAddLabel(list, tr("Weekly Task Expansion"), hasPermanentWeeklyTaskExpansion and tr("Yes") or tr("No"))
  end

  -- Outras compras
  storeSummaryAddSection(list, tr("Other Purchases"))
  storeSummaryAddLabel(list, tr("Instant Reward Access"), instantRewards or 0)
  storeSummaryAddLabel(list, tr("Charm Expansion"), hasCharmExpansion and tr("Yes") or tr("No"))
  storeSummaryAddLabel(list, tr("Hirelings"), hirelingsObtained or 0)
  if hirelingSkills and #hirelingSkills > 0 then
    storeSummaryAddLabel(list, tr("Hireling Skills"), #hirelingSkills)
  end

  -- Itens de casa comprados na store
  if houseItems and #houseItems > 0 then
    storeSummaryAddSection(list, tr("Store House Items"))
    for _, hi in ipairs(houseItems) do
      local name = hi[2] or hi.name or ("Item " .. tostring(hi[1] or hi.itemId or 0))
      local count = hi[3] or hi.count or 1
      storeSummaryAddLabel(list, name, count)
    end
  end
end
...
function Character.onCyclopediaInspect(items, playerName, outfit, playerInfo)
	...
	for _, data in pairs(items or {}) do
		local slot = data.slot or data[1]
		if slot then
			local normalizedDescriptions = {}
			for _, descriptionData in pairs(data.descriptions or data[5] or {}) do
				normalizedDescriptions[#normalizedDescriptions + 1] = {
					label = descriptionData.label or descriptionData[1] or "",
					content = descriptionData.content or descriptionData[2] or ""
				}
			end

			local normalizedImbuements = {}
			for _, imbuementId in pairs(data.imbuingSlots or data.imbuements or data[4] or {}) do
				local normalizedId = type(imbuementId) == "table" and (imbuementId.imbuimentId or imbuementId.imbuementId or imbuementId[1]) or imbuementId
				normalizedImbuements[#normalizedImbuements + 1] = { imbuimentId = normalizedId or 0 }
			end

			normalizedItems[slot] = {
				itemName = data.itemName or data.name or data[2],
				item = data.item or data[3],
				imbuingSlots = normalizedImbuements,
	...
	for _, data in pairs(playerInfo or {}) do
		normalizedInfo[#normalizedInfo + 1] = normalizeInspectionDescription(data)
	end

	inspectPlayer = {name = playerName, outfit = outfit, playerData = normalizedInfo}
	inspectItems = normalizedItems
end
...
local function setAppearanceOutfit(widget, outfitId, addons, colors)
    if not widget then
        return
    end
    ...
    if colors then
        outfitData.head = colors[1] or 0
        outfitData.body = colors[2] or 0
        outfitData.legs = colors[3] or 0
        outfitData.feet = colors[4] or 0
    end

    widget:setOutfit(outfitData)
end
    ...
    for _, data in pairs(currentList) do
        ...
        if outfit or mount then
            ...
            if outfit then
                ...
                setAppearanceOutfit(widget.outfit, data[1], data[3], appearancesList.outfitColors and appearancesList.outfitColors[1])
            elseif mount then
                widget.store:setVisible(data[3] == 2)
                setAppearanceOutfit(widget.outfit, data[1], 0, appearancesList.mountColors and appearancesList.mountColors[1])
            end
        end

        if familiar then
            setAppearanceOutfit(widget.outfit, data[1], 0)
            widget.name:setText(data[2])
            if data[3] == 2 then
...
function Character.onCyclopediaAchievements(achievementPoints, achievementSecrets, achievements)
	...
	local secretTotal = math.max(achievementSecrets or 0, 1)
	achievementWindow:recursiveGetChildById('secretLabel'):setText(secretAchievement .. '/' .. (achievementSecrets or 0))
	local secretProgress = achievementWindow:recursiveGetChildById('secretProgress')
	secretProgress:setPercent((secretAchievement / secretTotal) * 100)
