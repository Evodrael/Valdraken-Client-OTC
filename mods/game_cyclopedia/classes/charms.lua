...
function Charm:configureCharmPanel()
    ...
    for _, charmData in pairs(self.data) do
        ...
        if charm then
            ...
            if charmData.level and charmData.level > 0 then
...
function Charm:configureCreatureList(monsters)
    ...
    for monsterId, charmId in pairs(monsters) do
        ...
        if type(monster) == "table" then
            monsterName = monster.name or monster[1]
        end

        if type(monsterName) == "string" and #monsterName > 0 then
...
function Charm.onCharmData(resetAllCharmPrice, charmData, emptySlots, monsters)
    ...
    -- finishedMonsters chega como array sequencial (1->raceId), mas os consumidores
    -- (configureCreatureList/selectCreature) indexam por raceId. Converte para mapa.
    local monsterMap = {}
    if type(monsters) == "table" then
        for _, raceId in ipairs(monsters) do
            monsterMap[raceId] = raceId
        end
    end
    Charm.monsters = monsterMap
...
function Charm:setupContentPanel(widget)
    ...
    if player and (self.resetPrice == 0 or self.resetPrice > (player:getResourceValue(ResourceBank) + player:getResourceValue(ResourceInventary))) then
