...
local checkImages = {
	[true] = "/mods/game_cyclopedia/images/ui/check",
...
function Titles.parseData(currentTitle, list)
	...
	for _, data in pairs(list or {}) do
		if type(data) == "table" then
			data.id = data.id or data[1]
			data.name = data.name or data[2] or ""
			data.description = data.description or data[3] or ""
			if data.permanent == nil then
				data.permanent = data[4] == true or data[4] == 1
			end
			if data.unlocked == nil then
				data.unlocked = data[5] == true or data[5] == 1
			end
		end
	end
	currentTitleList = list or {}
	table.sort(currentTitleList, function(a, b)
		local left = type(a) == "table" and a.name or ""
		local right = type(b) == "table" and b.name or ""
