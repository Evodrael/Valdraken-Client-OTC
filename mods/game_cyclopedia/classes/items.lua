...
local marketItemCount = 0
local lastSelectedItem = nil
local lastSelectedCategory = nil
local oldBuyChild = nil
local oldSaleChild = nil
local itemsData = {}
local pendingLoadRetries = 0
local pendingLoadEvent = nil
local suppressItemSearchCallback = false
...
local function isItemPanelVisible()
	return VisibleCyclopediaPanel
		and VisibleCyclopediaPanel.leftInfo
		and VisibleCyclopediaPanel.leftInfo.categoriesList
		and VisibleCyclopediaPanel.leftInfo.itemList
end

local function setItemSearchTextSilently(text)
	if not VisibleCyclopediaPanel or not VisibleCyclopediaPanel.leftInfo then
		return
	end

	local searchText = VisibleCyclopediaPanel.leftInfo.searchText
	if not searchText then
		return
	end

	suppressItemSearchCallback = true
	searchText:setText(text or "", true)
	suppressItemSearchCallback = false
end

local function getCategoryId(categoryWidget)
	if not categoryWidget then
		return nil
	end

	local categoryId = categoryWidget.categoryId or categoryWidget:getActionId()
	return tonumber(categoryId) or categoryId
end

local function resetItemFilters()
	sortButtons["levelButton"] = false
	sortButtons["vocButton"] = false
	sortButtons["oneHandButton"] = false
	sortButtons["twoHandButton"] = false
	sortButtons["classOptions"] = -1

	if VisibleCyclopediaPanel and VisibleCyclopediaPanel.leftInfo then
		for _, id in ipairs({ "levelButton", "vocButton", "oneHandButton", "twoHandButton" }) do
			local button = VisibleCyclopediaPanel.leftInfo:getChildById(id)
			if button then
...
local function normalizeRestrictVocation(restrictVocation)
	if type(restrictVocation) == "table" then
		return restrictVocation
	end
...
local function areThingsLoaded()
	local thingsLoaded = not modules.game_things or not modules.game_things.isLoaded or modules.game_things.isLoaded()
	local usesLegacyDat = g_game.getFeature and g_game.getFeature(GameLoadSprInsteadProtobuf)
	if not thingsLoaded then
		return false
	end

	return not usesLegacyDat or not g_things.isDatLoaded or g_things.isDatLoaded()
end

local function ensureMarketItemsLoaded()
	if marketItemsLoaded and marketItemCount > 0 then
		return true
	end

	if not areThingsLoaded() then
		return false
	end

	CyclopediaItems.loadItems()
	return marketItemsLoaded and marketItemCount > 0
end

function CyclopediaItems.terminate()
	if pendingLoadEvent then
		removeEvent(pendingLoadEvent)
		pendingLoadEvent = nil
	end
...
function CyclopediaItems.onThingsLoaded()
	marketItemsLoaded = false
	marketItemCount = 0
	CyclopediaItems.loadItems()
	if isItemPanelVisible() then
...
function CyclopediaItems.loadItems()
	...
	marketItems[30] = marketItems[30] or {}
	...
	marketItems[MarketCategory.WeaponsAll] = marketItems[MarketCategory.WeaponsAll] or {}
  	...
  	local unsorted = g_game.getUnsortedCyclopediaItems() or {}
	...
	for i = 1, #types do
		...
		if item then
			...
			if not table.empty(marketData) then
				marketData.restrictVocation = normalizeRestrictVocation(marketData.restrictVocation)
				...
				item:setId(marketData.showAs > 0 and marketData.showAs or itemType:getId())
				...
				if marketItems[marketData.category] ~= nil then
					...
					marketItemCount = marketItemCount + 1
        		end
			end
		end
	end

	-- Coins (gold/platinum/crystal) are included via findThingTypeByAttr if the appearances.dat has market flags.
...
function CyclopediaItems.showCategories()
	if not isItemPanelVisible() then
		return
	end
	if not ensureMarketItemsLoaded() then
		pendingLoadRetries = pendingLoadRetries + 1
		if pendingLoadRetries <= 120 then
			if pendingLoadEvent then
				removeEvent(pendingLoadEvent)
			end
			pendingLoadEvent = scheduleEvent(function()
				pendingLoadEvent = nil
				if isItemPanelVisible() then
					CyclopediaItems.showCategories()
				end
			...
			g_logger.warning("[CyclopediaItems] item tab opened before market things were available")
		end
		return
	end
	if pendingLoadEvent then
	...
	local previousCategoryId = getCategoryId(lastSelectedCategory)
	local previousItemId = lastSelectedItem and lastSelectedItem.item and lastSelectedItem.item:getItemId() or nil
	...
	for _, pair in ipairs(categoryList) do
		if marketItems[pair[1]] and #marketItems[pair[1]] > 0 then
			...
			widget.onClick = function()
				...
				VisibleCyclopediaPanel.leftInfo.categoriesList:focusChild(widget)
				CyclopediaItems.categoryListChildFocus(VisibleCyclopediaPanel.leftInfo.categoriesList, widget)
			end
			colorCount = colorCount + 1
		end
	end
	VisibleCyclopediaPanel.leftInfo.categoriesList:updateScrollBars()
  ...
  local firstCategory = VisibleCyclopediaPanel.leftInfo.categoriesList:getFirstChild()
  local categoryToFocus = firstCategory
  if previousCategoryId then
	for _, child in ipairs(VisibleCyclopediaPanel.leftInfo.categoriesList:getChildren()) do
		if getCategoryId(child) == previousCategoryId then
			categoryToFocus = child
			break
		end
	end
  end
	if categoryToFocus then
    categoryToFocus.restoreItemId = previousItemId
	VisibleCyclopediaPanel.leftInfo.categoriesList:focusChild(categoryToFocus)
	if VisibleCyclopediaPanel.leftInfo.itemList:getChildCount() == 0 then
		CyclopediaItems.categoryListChildFocus(VisibleCyclopediaPanel.leftInfo.categoriesList, categoryToFocus)
	end
	scheduleEvent(function()
		if not isItemPanelVisible() then
			return
		...
		resetItemFilters()
		for _, child in ipairs(VisibleCyclopediaPanel.leftInfo.categoriesList:getChildren()) do
			local categoryId = getCategoryId(child)
			if categoryId and marketItems[categoryId] and #marketItems[categoryId] > 0 then
				VisibleCyclopediaPanel.leftInfo.categoriesList:focusChild(child)
				CyclopediaItems.categoryListChildFocus(VisibleCyclopediaPanel.leftInfo.categoriesList, child)
				break
			end
		end
	end, 50)
  else
	VisibleCyclopediaPanel.emptyLabel:setVisible(true)
	VisibleCyclopediaPanel.emptyLabel:setText("No cyclopedia items available.")
	g_logger.warning(string.format("[CyclopediaItems] no categories rendered with marketItemCount=%d", marketItemCount))
  end
...
function CyclopediaItems.categoryListChildFocus(self, selected)
	...
	if not ensureMarketItemsLoaded() then return end
	local selectedCategoryId = getCategoryId(selected)
	...
	VisibleCyclopediaPanel.leftInfo.itemList.onChildFocusChange = function(self, selected) CyclopediaItems.itemListChildFocus(self, selected) end
	local categoryItems = marketItems[selectedCategoryId] or {}
	for i, itemInfo in ipairs(categoryItems) do
	...
	local childToFocus = firstChild
	if selected.restoreItemId then
		for _, child in ipairs(VisibleCyclopediaPanel.leftInfo.itemList:getChildren()) do
			if child.item and child.item:getItemId() == selected.restoreItemId then
				childToFocus = child
				break
			end
		end
		selected.restoreItemId = nil
	end
	if childToFocus then
		VisibleCyclopediaPanel.leftInfo.itemList:focusChild(childToFocus)
		CyclopediaItems.itemListChildFocus(VisibleCyclopediaPanel.leftInfo.itemList, childToFocus)
	end
...
function CyclopediaItems.itemListChildFocus(self, selected)
  ...
  applyRarityFrame(VisibleCyclopediaPanel.leftInfo.imageWidget.itemImage)
...
function CyclopediaItems.updateDropTracker(widget, checked)
	...
	local targetChecked = not checked
	if not modules.game_analyser or not modules.game_analyser.managerDropTracker then
		widget:setChecked(checked, true)
		return
	end

	lastSelectedItem.name:setColor(targetChecked and "#FF9854" or "#c0c0c0")
	modules.game_analyser.managerDropTracker(lastSelectedItem.item:getItem():getId(), targetChecked)
	widget:setChecked(targetChecked, true)
end
...
function CyclopediaItems.showNpcData(item)
	...
	elseif item and item.getNpcSaleData then
		data = item:getNpcSaleData() or {}
	end
...
function CyclopediaItems.showItemPrice(item)
	...
	if item and item.getAverageMarketValue then
		avgMarket = item:getAverageMarketValue() or 0
	elseif item and item.getMeanPrice then
		avgMarket = item:getMeanPrice() or 0
	end
	...
	local itemDefaultValue = item and item.getDefaultValue and item:getDefaultValue() or (item and item.getPriceValue and item:getPriceValue() or 0)
	local resulting = isMarketPrice and avgMarket or itemDefaultValue
	if resulting == 0 then
		resulting = avgMarket
	end
	...
	if ItemsDatabase and ItemsDatabase.setRarityItem then
		ItemsDatabase.setRarityItem(VisibleCyclopediaPanel.leftInfo.imageWidget.itemImage, item, resulting)
		if lastSelectedItem and lastSelectedItem.item then
			ItemsDatabase.setRarityItem(lastSelectedItem.item, item, resulting)
		end
	end
...
function CyclopediaItems.showItemDescription(desc)
	...
	for _, data in pairs(desc) do
		...
		local label = data.detail or data.label or data[1] or ""
		local content = data.description or data.content or data[2] or ""
...
function CyclopediaItems.onSearch(widget)
	if suppressItemSearchCallback or not isItemPanelVisible() then
		return
	end
	...
	for c = MarketCategory.First, MarketCategory.Last do
		...
		for i, itemInfo in ipairs(marketItems[c] or {}) do
			if count >= 200 then
				break
			end
...
function CyclopediaItems.showSearchResult(list)
	...
	local firstChild = VisibleCyclopediaPanel.leftInfo.itemList:getChildren()[1]
	if firstChild then
		VisibleCyclopediaPanel.leftInfo.itemList:focusChild(firstChild)
		CyclopediaItems.itemListChildFocus(VisibleCyclopediaPanel.leftInfo.itemList, firstChild)
	end
end
...
function CyclopediaItems.checkSortOptions(itemData)
	...
	if sortButtons["vocButton"] then
		local itemVocation = normalizeRestrictVocation(itemData.marketData.restrictVocation)
...
function CyclopediaItems.onSortFields(widget, checked)
	...
	if widget:getId() ~= "classOptions" then
		...
		sortButtons["classOptions"] = checked > 1 and (checked - 2) or -1
	end

	VisibleCyclopediaPanel.leftInfo.itemList:destroyChildren()
	local categoryItems = marketItems[lastSelectedCategory:getActionId()] or {}
	for i, itemInfo in ipairs(categoryItems) do
		if not CyclopediaItems.checkSortOptions(itemInfo) then
			goto Continue
		end
...
function CyclopediaItems.manageQuickloot(widget, checked)
	...
	local targetChecked = not checked
	if not modules.game_quickloot then
		widget:setChecked(checked, true)
		return true
	end

	if targetChecked then
...
function CyclopediaItems.onUpdateResultingValue(value)
	...
	if ItemsDatabase and ItemsDatabase.setRarityItem and lastSelectedItem and lastSelectedItem.item then
		ItemsDatabase.setRarityItem(lastSelectedItem.item, lastSelectedItem.item:getItem(), resulting)
		ItemsDatabase.setRarityItem(VisibleCyclopediaPanel.leftInfo.imageWidget.itemImage, lastSelectedItem.item:getItem(), resulting)
	end
end
...
function CyclopediaItems.onChangeCustomPrice(widget)
	...
	if #currentText == 0 then
		...
		local itemDefaultValue = item.getDefaultValue and item:getDefaultValue() or (item.getPriceValue and item:getPriceValue() or 0)
