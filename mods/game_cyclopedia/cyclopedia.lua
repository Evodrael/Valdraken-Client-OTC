...
local pendingTabSwitch = nil
local TAB_CLICK_THROTTLE_MS = 250
...
function init()
  ...
  connect(g_game, {
    ...
    onParseCyclopediaHousesInfo = House.onParseCyclopediaHousesInfo,
    onParseCyclopediaHouseList = House.onParseCyclopediaHouseList,
    ...
    onCyclopediaCharacterOffenceStats = Character.onCyclopediaOffenceStatsData,
    onCyclopediaCharacterDefenceStats = Character.onCyclopediaDefenceStatsData,
    onCyclopediaCharacterMiscStats = Character.onCyclopediaMiscStatsData,
    onCyclopediaMapData = MapCyclopedia.onCyclopediaMapData,
    ...
    onParseCyclopediaStoreSummary = Character.onParseCyclopediaStoreSummary,
  ...
  connect(g_things, {
    onLoadDat = CyclopediaItems.onThingsLoaded,
    onLoadStaticData = CyclopediaItems.onThingsLoaded
  })
...
-- Abre a Cyclopedia direto na aba "Magical Archive". Usado pelo botao da SpellList.
function Cyclopedia.openMagicalArchive()
  Cyclopedia:open()
  if not cyclopediaOptionsPanel then
    return
  end
  for _, child in pairs(cyclopediaOptionsPanel:getChildren()) do
    if child.category:getText() == 'Magical Archive' then
      onOptionChange(child)
      break
    end
  end
...
function Cyclopedia.close()
  if VisibleCyclopediaPanel then
    local minimap = VisibleCyclopediaPanel:recursiveGetChildById('minimap')
    if minimap and minimap.resetCustomMouseEvent then
      minimap:resetCustomMouseEvent()
    end
  end
  ...
  -- Sincroniza o side button: fechar pela propria janela (X/Esc) deixava o botao
  -- "cyclopediaDialog" afundado/checado. No proximo clique o sidebuttons achava que
  -- a janela ainda estava aberta e caia no fluxo de FECHAR (safeClose) em vez de
  -- reabrir -> sintoma "a Cyclopedia nao abre mais depois que fecho".
  if modules.game_sidebuttons and modules.game_sidebuttons.setButtonChecked then
    modules.game_sidebuttons.setButtonChecked("cyclopediaDialog", false)
  end
end
...
local function refreshHeaderBalances(requestServerData, withCharmBalances)
  if not cyclopediaWindow or not cyclopediaWindow:isVisible() or not g_game or not g_game.isOnline or not g_game.isOnline() then
    return
  end

  if requestServerData then
    g_game.requestResource(ResourceBank)
    g_game.requestResource(ResourceInventary)
    if withCharmBalances then
...
function onOptionChange(widget)
  ...
  -- the user spam-clicks tabs (each switch fires multiple requestResource /
  ...
  elseif currentTime - lastTabClickTime < TAB_CLICK_THROTTLE_MS then
    -- Coalesce: remember the latest requested tab and apply it once the
    -- throttle clears, so the final click always lands on the right panel.
    if pendingTabSwitch then
      removeEvent(pendingTabSwitch)
    end
    local targetWidget = widget
    pendingTabSwitch = scheduleEvent(function()
      pendingTabSwitch = nil
      if cyclopediaWindow and cyclopediaWindow:isVisible() and targetWidget and not targetWidget:isDestroyed() then
        onOptionChange(targetWidget)
      end
    end, TAB_CLICK_THROTTLE_MS - (currentTime - lastTabClickTime))
    return
  end
  ...
  if not selectedOption or selectedOption ~= widget:getId() then
    ...
    if widget.category:getText() == "Houses" then
      cyclopediaWindow.coinsStatus.imageGold:setImageSource('/mods/game_cyclopedia/images/ui/bank')
      cyclopediaWindow.coinsStatus.imageGold:setMarginTop(3)
    else
      cyclopediaWindow.coinsStatus.imageGold:setImageSource('/mods/game_cyclopedia/images/ui/gold')
      cyclopediaWindow.coinsStatus.imageGold:setMarginTop(5)
    end

    if bntstatus.coinsStatus or bntstatus.charmStatus or bntstatus.minorCharmStatus then
      refreshHeaderBalances(true, bntstatus.charmStatus or bntstatus.minorCharmStatus)
    end
  end
  ...
  elseif widget.category:getText() == 'Houses' then
    ...
    local townFilter = VisibleCyclopediaPanel.checkboxBackground.ownHouses:getCurrentOption().text
    House.selectTown(townFilter)
