...
function ImbuementScroll.setup(availableImbuements, needItems)
    ...
    -- Diagnostic: print every imbue the server sent and what tier ("type")
    -- it carries, so any "missing" imbuement can be tracked back to either
    -- the server payload or the type==baseImbuement filter in selectBaseType.
    local count = 0
    for id, imb in pairs(self.availableImbuements) do
        count = count + 1
        print(string.format('[ImbueShrine.Scroll] id=%s name=%s type=%s imageId=%s cost=%s',
            tostring(id), tostring(imb.name), tostring(imb.type),
            tostring(imb.imageId), tostring(imb.cost)))
    end
    print(string.format('[ImbueShrine.Scroll] total imbuements received: %d', count))
...
function ImbuementScroll.selectBaseType(selectedButtonId)
    ...
    for id, imbuement in ipairs(self.availableImbuements) do
        if imbuement.type == baseImbuement then
            ...
            -- Switched to per-imbuement PNGs (ported from LuminarisOT). The
            -- single-strip approach with getFramePosition was clipping to
            -- out-of-strip coordinates for any imbuement.imageId whose row
            -- wasn't physically present, producing empty boxes in the picker.
