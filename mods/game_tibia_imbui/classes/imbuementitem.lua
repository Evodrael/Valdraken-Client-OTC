...
function ImbuementItem.setup(itemId, tier, slots, activeSlots, availableImbuements, needItems)
    ...
    -- Diagnostic: same tracking as ImbuementScroll.setup. The list shown in
    -- the "Select an Imbuement" panel is filtered by `imbuement.type ==
    -- baseImbuement` (Basic=0 / Intricate=1 / Powerful=2). If the screenshot
    -- shows empty boxes, this log makes it obvious whether the server didn't
    -- send those imbuements at all or sent them with the wrong tier.
    do
        local count = 0
        for id, imb in pairs(availableImbuements or {}) do
            count = count + 1
            print(string.format('[ImbueShrine.Item] id=%s name=%s type=%s imageId=%s cost=%s',
                tostring(id), tostring(imb.name), tostring(imb.type),
                tostring(imb.imageId), tostring(imb.cost)))
        end
        print(string.format('[ImbueShrine.Item] total imbuements received: %d (item tier=%s)',
            count, tostring(tier)))
    end
...
function ImbuementItem.configureWindow(window)
    ...
    for i = 1, 3 do
        ...
        if slotWidget then
            -- Default empty-slot icon (use the strip's first 64x64 cell as the
            -- placeholder); replaced with the per-imbuement PNG below if the
            -- slot has an active imbue.
            ...
            if i <= self.slots then
                ...
                if imbuement and imbuement[1] and imbuement[1].id ~= 0 then
                    slotWidget.resource:setImageSource("/images/game/imbuing/icons/" .. tostring(imbuement[1]["imageId"]))
                    slotWidget.resource:setImageClip("0 0 64 64")
                end
            else
    ...
    if itemWidget then
        ...
        itemName = itemWidget:getItem() and itemWidget:getItem():getName() or itemName
    end
...
function ImbuementItem.onSelectSlotClear(imbuement)
    ...
    widget.resource:setImageSource("/images/game/imbuing/icons/" .. tostring(imbuement[1]["imageId"]))
    widget.resource:setImageClip("0 0 64 64")
    ...
    if clearButton then
        ...
        clearButton.onClick = function()
            ...
            g_client.setInputLockWidget(self.confirmWindow)
        end
...
function ImbuementItem.selectBaseType(selectedButtonId)
    ...
    for id, imbuement in pairs(self.availableImbuements) do
        if imbuement.type == baseImbuement then
            ...
            widget.resource:setImageSource("/images/game/imbuing/icons/" .. tostring(imbuement.imageId))
            widget.resource:setImageClip("0 0 64 64")
...
function ImbuementItem.selectImbuementWidget(widget, imbuement)
            ...
            if itemWidget then
                ...
                if source then
                    ...
                    if self.needItems[source.item:getId()] >= source.item:getCount() then
                        ...
                        itemWidget.count:setColor("$var-text-cip-color-light-red")
                    end
    ...
    if costPanel then
        ...
        costPanel.cost:setColor(balance < cost and "$var-text-cip-color-light-red" or "$var-text-cip-color")
    end
