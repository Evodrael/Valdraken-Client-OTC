...
function BossCooldown:setupCooldown(cooldown)
	-- O C++ (parseBosstiaryCooldownTimer) envia uma tabela com chaves NOMEADAS
	-- {bossRaceId=, cooldownTime=} e NAO array posicional, e nao manda name/outfit.
	-- Resolvemos nome e outfit localmente via g_things.getMonsterList()[raceId]
	-- (mesmo padrao do bosstiary.lua). Antes lia cooldown[1..4] -> tudo nil ->
	-- setOutfit(nil) quebrava com "cast a 'nil' lua value to 'class Outfit'".
	BossCooldown.cooldown = {}
	local monsterList = g_things.getMonsterList()
	for _, entry in pairs(cooldown) do
		local raceId = entry.bossRaceId or entry[1]
		local cd = entry.cooldownTime or entry[2]
		if raceId then
			local race = monsterList[raceId]
			if race then
				local outfit = {
					type = race[2] or 0,
					auxType = race[3] or 0,
				...
				if outfit.auxType ~= 0 then
					outfit.category = ThingCategoryItem
				else
					outfit.category = ThingCategoryCreature
				end
				BossCooldown.cooldown[#BossCooldown.cooldown + 1] =
					{bossId = raceId, cooldown = cd, name = race[1] or tostring(raceId), outfit = outfit}
			end
		end
	end
...
function toggleBossCDFocus(visible)
	...
	elseif widget then
		...
		widget.onClick = function() m_interface.toggleInternalFocus(); toggleBossCDFocus(not visible) end
	end
	m_interface.toggleFocus(visible, "bosscooldown")
	if visible then
