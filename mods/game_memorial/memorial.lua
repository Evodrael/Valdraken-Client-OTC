...
-- Converte uma lista de nomes (array de string vinda do C++) para o formato
-- {{name=...}, ...} que selectGoldenPanel/selectRoyalPanel esperam.
local function wrapNames(list)
	local t = {}
	if list then
...
-- Assinatura vinda de ProtocolGame::parseOutfitMemorial (opcode 0xB0):
-- golden: 3 precos + 3 listas (basic, one-addon, full)
-- royal: 3 pares (silver, gold) + 3 listas (basic, one-addon, full)
function onOutfitMemorial(gBasePrice, gFirstPrice, gSecondPrice, gBaseList, gFirstList, gSecondList,
						   rBaseSilver, rBaseGold, rFirstSilver, rFirstGold, rSecondSilver, rSecondGold,
						   rBaseList, rFirstList, rSecondList)
	goldenInfo = {
		basePrice = gBasePrice,
		firstPrice = gFirstPrice,
		secondPrice = gSecondPrice,
		baseOutfit = wrapNames(gBaseList),
		firstAddon = wrapNames(gFirstList),
		secondAddon = wrapNames(gSecondList),
	}
	royalInfo = {
		baseSilver = rBaseSilver, baseGold = rBaseGold,
		firstSilver = rFirstSilver, firstGold = rFirstGold,
		secondSilver = rSecondSilver, secondGold = rSecondGold,
		baseOutfit = wrapNames(rBaseList),
		firstAddon = wrapNames(rFirstList),
		secondAddon = wrapNames(rSecondList),
	}
