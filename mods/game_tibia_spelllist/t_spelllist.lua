...
-- Cache de precos (goldPrice) lido do JSON da Magical Archive, indexado por WORDS (formula).
-- Casamos por words porque o spell id do servidor e o spellid do JSON sao espacos diferentes.
local spellPricesByWords = nil

local function loadSpellPrices()
  if spellPricesByWords then
    return spellPricesByWords
  end
  spellPricesByWords = {}
  local file = "/mods/game_cyclopedia/classes/magical_archive/spells.json"
  if g_resources.fileExists(file) then
    ...
    if ok and type(data) == "table" then
      for _, s in ipairs(data) do
        local w = s.formulaWithoutParams
        if w and #w > 0 then
          spellPricesByWords[string.trim(string.lower(w))] = s.goldPrice or 0
        end
      end
    end
  end
...
local function getSpellPrice(spell)
  if not spell or not spell.words then
    return 0
  end
  return loadSpellPrices()[string.trim(string.lower(spell.words))] or 0
end
...
function onSpellsChange(player, list)
  ...
  for _, spellId in pairs(list) do
    ...
    if spell then
      ...
      spell.price = getSpellPrice(spell)
...
function onSelectedSpell(list, focused, oldFocus)
  ...
  t_spelllist:recursiveGetChildById('price'):setText(comma_value(focused.spellData.price or 0))
...
function matchFilter(spell)
  ...
  -- Filtro premium/free: esconde premium quando "Premium Account" esta off, e free quando
  -- "Free Account" esta off (antes escondia TODAS as spells quando showFreeSpells era off).
  if spell.premium == true then
    if not getSpellOption("showPremiumOnlySpells") then
      return false
    end
  else
