...
-- true durante o foco PROGRAMATICO do startup (onShowPodiumOutfits). Impede que o
-- foco inicial auto-marque showOutfitCheck e sobrescreva o estado salvo do podium
-- (ex.: podium so de mount). No foco do USUARIO (clicar num outfit) fica false ->
-- auto-marca "Outfit" (mostra no preview E salva o outfit, senao type=0 = podium vazio).
local applyingStartup = false
...
function init()
    ...
    connect(g_game, { onParsePlayerPodium = onParsePlayerPodium, onGameEnd = close })
end

function terminate()
    PodiumWindow:hide()
    disconnect(g_game, { onParsePlayerPodium = onParsePlayerPodium, onGameEnd = close })
end

-- Sinal vindo do C++ (processOpenPodiumWindow / 0xC8 modo 5). Monta a tabela `data`
-- e abre a janela. O servidor (CipSoft puro) envia apenas a lista do sexo atual e
-- nao envia auras -> maleOutfits/femaleOutfits/auraList ficam vazios.
function onParsePlayerPodium(currentOutfit, outfitList, mountList, pos, itemId, showPlatform, showOutfit, direction, mounted)
    local data = {
        itemId = itemId,
        pos = pos,
        direction = direction,
        ...
        showPodium = showPlatform,
        aura = false,
    }
    onOpenPodiumWindow(currentOutfit, outfitList, mountList, data, {}, {}, {})
end
...
function onOpenPodiumWindow(outfit, outfitList, mountList, data, maleOutfits, femaleOutfits, auraList)
    ...
    for _, outfitData in pairs(outfitList) do
        ...
        -- Fallback (CipSoft puro): sem listas de pareamento macho/femea, o servidor
        -- envia apenas a lista do sexo atual. Mostramos o outfit como veio.
        table.insert(availableOutfits, {name = outfitName, male = outfitId, female = outfitId, addons = addons})
...
function onShowPodiumOutfits(startup, searchText)
    ...
    if startup then
        -- dontSignal=true: selecionar a aba "Outfit" no startup NAO pode disparar
        -- onChangeOptions -> onShowPodiumOutfits de novo (re-entrancia que destruia e
        -- reconstruia a lista, deixando currentOutfitChild/targetFocus apontando para
        -- widgets ja destruidos e quebrando o foco/click na selecao de outfit).
        optionsRadio:selectWidget(PodiumWindow:recursiveGetChildById("outfitCheck"), true)
    ...
    -- foco programatico: nao deixar onOutfitFocusChange auto-marcar "Outfit"
...
function onOutfitFocusChange(list, focused)
    ...
    -- Clique do usuario num outfit implica exibi-lo: marca "Outfit" (showOutfitCheck).
    -- Sem isto, num podium novo o check fica desmarcado -> preview oculto E o save
    -- manda type=0 (podium vazio in-game). applyingStartup evita marcar no foco inicial.
    if not applyingStartup and not showOutfitCheck:isChecked() then
