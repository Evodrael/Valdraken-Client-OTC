...
-- ===================== PROTOCOLO BINARIO DO CHARACTER AUCTION =====================
-- O servidor (data/modules/scripts/character_auctions/character_auction.lua) NAO
-- usa extended opcode/JSON: a comunicacao e BINARIA.
--   Cliente -> Servidor: recvbyte 0x76 (118)
--   Servidor -> Cliente: header 0x80 (128)
-- Subtipos enviados pelo servidor (header type): 0=MainWindow, 1=DepotAndInventory,
-- 2=Store, 3=Arguments, 4=FeeAndTexts.
-- Subtipos enviados pelo cliente (1o byte apos 0x76): 0=abrir, 2=preview de taxa,
-- 3=confirmar venda.
Bazaar.OPCODE_SEND = 0x76
Bazaar.OPCODE_RECV = 0x80

-- header recebido (server -> client)
Bazaar.RECV_MAIN = 0
Bazaar.RECV_DEPOT = 1
Bazaar.RECV_STORE = 2
Bazaar.RECV_ARGUMENTS = 3
Bazaar.RECV_FEE = 4

-- subtipo enviado (client -> server)
Bazaar.SEND_OPEN = 0
Bazaar.SEND_FEE = 2
Bazaar.SEND_CONFIRM = 3

-- Itens carregam o byte de tier apenas quando o item tem classification > 0
-- (mesma regra que o servidor usa: ItemType:getClassification() > 0).
local function itemHasTier(clientId)
	local thingType = g_things.getThingType(clientId, ThingCategoryItem)
	return thingType ~= nil and thingType:getClassification() > 0
end

-- Le uma lista de itens (Depot/Inventory ou Store):
-- U16 count; por item: U16 clientId, [U8 tier se classification>0], U32 count.
local function readAuctionItemList(msg)
	local items = {}
	local count = msg:getU16()
	for _ = 1, count do
		local clientId = msg:getU16()
		local tier = 0
		if itemHasTier(clientId) then
			tier = msg:getU8()
		end
		local amount = msg:getU32()
		items[#items + 1] = { clientId, tier, amount }
	end
	return items
end

-- Handler do opcode binario 0x80. Substitui o antigo extended JSON 202; reaproveita
-- exatamente os mesmos callbacks (g_game.onCharacterBazar*) ja conectados no init.
function Bazaar.onAuctionOpcode(protocol, msg)
	local ptype = msg:getU8()
	if ptype == Bazaar.RECV_MAIN then
		...
		local worldType = msg:getU8()
		local minDuration = msg:getU32()
		local maxDuration = msg:getU32()
		local minValue = msg:getU16()
		local requeriments = {}
		local reqCount = msg:getU8()
		for _ = 1, reqCount do
			local text = msg:getString()
			local satisfied = msg:getU8() == 1
			requeriments[text] = satisfied
		end
		g_game.onCharacterBazarRequeriments(name, level, vocationId, outfit, worldType, minDuration, maxDuration, minValue, requeriments)
	elseif ptype == Bazaar.RECV_DEPOT then
		g_game.onCharacterBazarItems(readAuctionItemList(msg))
	elseif ptype == Bazaar.RECV_STORE then
		g_game.onCharacterBazarStoreItems(readAuctionItemList(msg))
	elseif ptype == Bazaar.RECV_ARGUMENTS then
		local arguments = {}
		local argCount = msg:getU8()
		for _ = 1, argCount do
			local arg = {}
			arg[1] = msg:getU8()       -- icon id
			arg[2] = msg:getString()   -- title
			arg[3] = {}                -- map: index global -> texto
			local entryCount = msg:getU16()
			for _ = 1, entryCount do
				local index = msg:getU16()
				arg[3][index] = msg:getString()
			end
			arguments[#arguments + 1] = arg
		end
		g_game.onCharacterBazarInformations(arguments)
	elseif ptype == Bazaar.RECV_FEE then
		local fee = msg:getU16()
		local errors = {}
		local errCount = msg:getU8()
		for _ = 1, errCount do
			errors[#errors + 1] = msg:getString()
		end
		g_game.onCharacterBazarCheckInformations(fee, errors)
	end
end

local function getAuctionProtocol()
	return g_game.getProtocolGame()
end

-- Escreve a selecao atual (bid, data, itens, store items, argumentos) no formato
-- esperado pelo servidor (parseMainWindowOpen tipos 2 e 3).
local function writeAuctionSelection(msg, sendType)
	msg:addU8(Bazaar.OPCODE_SEND)
	msg:addU8(sendType)
	msg:addU32(Bazaar.sellValue or 0)
	msg:addU32(Bazaar.selectedSlotDay or 0)

	-- itens selecionados (slots itemButton0..3)
	local items = {}
	for i = 1, 4 do
	...
	for _, item in ipairs(items) do
		...
		if itemHasTier(item:getId()) then
			msg:addU8(item:getTier())
		end
	end

	-- store items: este build nao tem slots separados de store items.
	msg:addU8(0)

	-- argumentos selecionados (indices globais)
	local args = {}
	for i = 1, 5 do
		if Bazaar.setArguments[i] then
			args[#args + 1] = Bazaar.setArguments[i]
		end
	end
	msg:addU8(#args)
...
function Bazaar.sendOpen()
	local protocol = getAuctionProtocol()
	if not protocol then
		return
	end
	local msg = OutputMessage.create()
	msg:addU8(Bazaar.OPCODE_SEND)
	msg:addU8(Bazaar.SEND_OPEN)
	protocol:send(msg)
end

function Bazaar.sendFeePreview()
	local protocol = getAuctionProtocol()
	if not protocol then
		return
	end
	local msg = OutputMessage.create()
	writeAuctionSelection(msg, Bazaar.SEND_FEE)
	protocol:send(msg)
end

function Bazaar.sendConfirm()
	local protocol = getAuctionProtocol()
	if not protocol then
		return
	end
	local msg = OutputMessage.create()
	writeAuctionSelection(msg, Bazaar.SEND_CONFIRM)
	protocol:send(msg)
end
...
function Bazaar:showRules()
	...
	-- No protocolo binario o servidor ja envia itens (Depot/Store) e argumentos
	-- logo apos o MainWindow (resposta unica de Bazaar.sendOpen), entao nao ha
	-- requests separados aqui. A taxa/erros sao buscados no passo de confirmacao
	-- via Bazaar.sendFeePreview().
...
function Bazaar:openConfirmation()
	...
	-- Busca a taxa real e eventuais erros de validacao (servidor responde com
	-- FeeAndTexts / header 0x80 subtipo 4 -> g_game.onCharacterBazarCheckInformations).
	Bazaar.sendFeePreview()
	...
	bazaarWindow.contentPanel.characterPanel.next.onClick = function()
		-- Confirmacao em binario (recvbyte 0x76, subtipo 3). A selecao atual
		-- (itens/argumentos/bid/data) e serializada dentro de Bazaar.sendConfirm().
		Bazaar.sendConfirm()
	end
	bazaarWindow.contentPanel.characterPanel.cancel.onClick = function() Bazaar:showInfo() end
