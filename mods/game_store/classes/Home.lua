...
local function timerEvent(widget, endTime)
	...
	-- When the offer window expires we ask the server for a fresh Home page so
	-- the next set of daily offers + reroll seed lands without requiring the
	-- player to close and reopen the store.
	if os.time() > endTime then
		widget:setText("Ends in: 00:00:00")
		if Store and Store.requestOffers then
			Store:requestOffers(OPEN_HOME, 0, 0)
		end
		return
	end
...
local function normalizeOffer(offer)
	...
	-- HomeOffer (Recently Added + Daily Offers) is parsed by the C++ side
	-- without any subOffers (see protocolgameparse.cpp readHomeOffer). If we
	-- leave offer.offers empty, the price/strikethrough rendering loop in
	-- createDailyOffers iterates zero times and the price labels stay blank.
	-- Synthesize one entry from the top-level HomeOffer fields so the
	-- existing rendering code works on daily offers as well.
	if #offer.offers == 0 and (offer.discountPrice or 0) > 0 then
		table.insert(offer.offers, {
			id = offer.id,
			price = offer.price or 0,
			count = 1,
			coinType = offer.coinType or 0,
			basePrice = offer.price or 0,
			saleValidUntilTimestamp = offer.expireTime or 0,
			disabledReasons = {},
			disabledReason = "",
		})
	end
...
local function getStoreImageUrl(image)
	image = image or ""
	if image:match("^https?://") then
		return image
	end

	local baseUrl = Store.url or ""
	if baseUrl == "" or baseUrl:sub(-1) == "/" or image:sub(1, 1) == "/" then
...
function HomeOffer:configure(categoryName, offers, scrolling, homePanel, reasons, dailyOfferPrice, dailyOffers)
	...
	HomeOffer.offers = offers or {}
	HomeOffer.scrolling = math.max(1000, (tonumber(scrolling) or 5) * 1000)
	HomeOffer.homePanel = homePanel or {}
	HomeOffer.dailyReroll = tonumber(dailyOfferPrice or 0) or 0
	HomeOffer.dailyOffers = {} -- Daily Offers removido: ignora ofertas diarias do servidor (bloco oculto em home.otui)

	Offers.reasons = reasons or {}
...
function HomeOffer:onRerollDailyOffer(button)
	...
	-- The Store window stays open while the confirmation dialog floats over
	-- it. Hiding it (previous behaviour) made the user lose context: after
	-- confirming the reroll the Store stayed closed, so they had to reopen
	-- it manually just to see the new offers. The input lock below keeps
	-- the user from clicking other offers behind the dialog.
...
function HomeOffer:createOffers()
	...
	for _, offer in ipairs(HomeOffer.offers) do
		...
		widget.onClick = function()
			Store:requestOffers(OPEN_OFFER, "", offer.id)
		end
...
function HomeOffer:configurePanels()
	...
	HTTP.downloadImage(getStoreImageUrl(homeInfo[1]), function(path, err)
		...
		if homeInfo[2] == 2 then
			Store:requestOffers(OPEN_CATEGORY, homeInfo[3], 0)
		end
