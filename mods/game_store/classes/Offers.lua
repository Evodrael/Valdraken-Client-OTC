...
local function normalizeOffer(offer)
	offer.icon = offer.icon or ""
	offer.itemId = tonumber(offer.itemId or offer.itemType or 0) or 0
	offer.itemType = tonumber(offer.itemType or offer.itemId or 0) or 0
	offer.mountId = tonumber(offer.mountId or offer.mountClientId or 0) or 0
	offer.offerType = tonumber(offer.offerType or 0) or 0
	offer.state = tonumber(offer.state or 0) or 0
	offer.filter = offer.filter or ""
	offer.offers = offer.offers or offer.subOffers or {}
	offer.bundles = offer.bundles or {}
	for _, subOffer in pairs(offer.offers) do
		subOffer.disabledReasons = subOffer.disabledReasons or {}
		subOffer.disabledReason = subOffer.disabledReason or ""
		subOffer.price = tonumber(subOffer.price or 0) or 0
		subOffer.count = tonumber(subOffer.count or 0) or 0
		subOffer.coinType = tonumber(subOffer.coinType or 0) or 0
		subOffer.basePrice = tonumber(subOffer.basePrice or 0) or 0
		subOffer.saleValidUntilTimestamp = tonumber(subOffer.saleValidUntilTimestamp or subOffer.validUntil or 0) or 0
	end
	return offer
end

local function getDisableReason(reasonId)
	reasonId = tonumber(reasonId) or 0
	return Offers.reasons[reasonId] or Offers.reasons[reasonId + 1] or Offers.reasons[reasonId - 1] or ""
end
...
function calldescription(offerId)
	...
	Offers.event =  scheduleEvent(function()
		local description = Offers.pendingDescriptions[offerId]
		if description then
		...
		if g_game.requestOfferDescription then
			...
			g_game.sendRequestStoreOfferById(offerId, 0, 0)
		end
		g_game.doThing(true)
...
function Offers:refreshOffers(displayOffer, redirect, filter)
	...
	for counter, offer in ipairs(displayOffer) do
		...
		if Offers.pendingDescriptions[offer.id] then
			scheduleEvent(function()
				Offers:configureDescription(offer.id, Offers.pendingDescriptions[offer.id])
			end, 1)
		end
		local color = ''
		...
		for i = #offer.offers, 1, -1 do
			...
			for _, i in pairs(subOffer.disabledReasons) do
				...
				local reason = getDisableReason(i.reasonId)
				if reason ~= "" then
					subOffer.disabledReason = string.format("%s* %s\n", subOffer.disabledReason, reason)
				end
			end
...
function Offers:onSelectionOffer(_, selectedWidget)
	...
	if offer.offers[1].disabledReason ~= '' then
		...
		local msg = "The product is not available for this character:\n" .. offer.offers[1].disabledReason
...
function Offers:configureDescription(offerId, description)
	...
	Offers.pendingDescriptions[offerId] = description
	if not Offers.clientOffers[offerId] then
		return true
	end
	...
	novo_texto = string.gsub(novo_texto, "{info}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="0 0 13 13" />')
	novo_texto = string.gsub(novo_texto, "{character}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="13 0 13 13" />only usable by purchasing character')
	novo_texto = string.gsub(novo_texto, "{activated}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="130 0 13 13" />activated at purchase')
	novo_texto = string.gsub(novo_texto, "{useicon}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="169 0 13 13" />')
	novo_texto = string.gsub(novo_texto, "{use}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="169 0 13 13" />use it to unwrap or activate this product')
	novo_texto = string.gsub(novo_texto, "{limit|(%d+)}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="78 0 13 13" />maximum amount that can be owned by character: %1')
	novo_texto = string.gsub(novo_texto, "{house}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="65 0 13 13" />can only be unwrapped in a house owned by the purchasing character')
	novo_texto = string.gsub(novo_texto, "{box}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="39 0 13 13" />comes in a box which can only be unwrapped by purchasing character')
	novo_texto = string.gsub(novo_texto, "{boxicon}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="39 0 13 13" />')
	novo_texto = string.gsub(novo_texto, "{storeinbox}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="52 0 13 13" />will be sent to your Store inbox and can only be stored there and in depot box')
	novo_texto = string.gsub(novo_texto, "{usablebyallicon}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="26 0 13 13" />')
	novo_texto = string.gsub(novo_texto, "{usablebyall}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="26 0 13 13" />can be used by all characters on this account')
	novo_texto = string.gsub(novo_texto, "{backtoinbox}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="91 0 13 13" />will be wrapped back and sent to inbox if the purchasing character is no longer the house owner')
	novo_texto = string.gsub(novo_texto, "{storeinboxicon}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="91 0 13 13" />')
	novo_texto = string.gsub(novo_texto, "{capacity}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="156 0 13 13" /><i>cannot be purchased if capacity is exceeded</i>')
	novo_texto = string.gsub(novo_texto, "{speedboost}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="117 0 13 13" />provides character with a speed boost')
	novo_texto = string.gsub(novo_texto, "{battlesign}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="143 0 13 13" />cannot be purchased by characters with protection zone block or battle sign')
	novo_texto = string.gsub(novo_texto, "{vocationlevelcheck}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="0 0 13 13" />vocation and level requirements apply')
	novo_texto = string.gsub(novo_texto, "{transferableprice}", '<img src="/images/store/icon-tibiacointransferable" width="12" height="12" />can be purchased with transferable Tibia Coins')
	novo_texto = string.gsub(novo_texto, "{once}", '<img src="/images/store/store-icons-inline" width="13" height="13" clip="78 0 13 13" />can only be purchased once')
	novo_texto = string.gsub(novo_texto, "{star}", '<img src="/images/store/icon-star-gold" width="9" height="10" />')
...
-- Anima um spritesheet horizontal (frames lado a lado) trocando o image-clip.
-- Definido ANTES de onStorePurchase para que a box possa abrir automaticamente
-- ao concluir a compra (e nao so ao clicar no confirm).
...
function onStorePurchase(message)
	...
	buyOfferWindow:hide()
	g_client.setInputLockWidget(nil)
	-- Toca a animacao da box ABRINDO automaticamente ao concluir a compra. Antes a
	-- box ficava estatica (frame idle) e so animava ao clicar no confirm, entao o
	-- usuario nao via a box abrir para "receber" o item. Agora abre sozinha.
	SucessOfferWindow.confirm.image:setImageSource('/images/store/purchasecomplete_pressed')
	SucessOfferWindow.confirm.image:setImageClip("0 0 108 108")
	animateImage(SucessOfferWindow.confirm.image, 108, 108, 1, 13, 60)
	SucessOfferWindow.description.message:setText(message)
	scheduleEvent(function() SucessOfferWindow:focus() end, 50)
end

-- Confirm/Escape da janela de sucesso: fecha e volta a loja. A animacao da box
-- abrindo agora toca automaticamente em onStorePurchase, entao aqui so fechamos
-- (sem re-animar / sem delay, evitando flicker e espera de 1s).
