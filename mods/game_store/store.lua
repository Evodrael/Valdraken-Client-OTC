...
local STORE_EXTENDED_OPCODE = 202
...
local function normalizeNumericKeyMap(entries)
  local normalized = {}
...
local function onStoreExtendedJSONOpcode(protocol, opcode, data)
  if not data or type(data) ~= "table" then
    return
  end

  -- Character auction (bazaar) agora e 100% BINARIO (opcode 0x80 recv / 0x76 send),
  -- tratado em Bazaar.onAuctionOpcode. Este canal JSON 202 cobre apenas PIX e loyalty.
  if data.action == "pix_data" then
    g_game.onRecvPixData(normalizeNumericKeyMap(data.pixList or {}))
  elseif data.action == "pix_url" then
    g_game.onRecvPixURL(data.url or "", data.token or "")
  ...
  elseif data.action == "loyalty_info" then
    ...
    -- plain `LoyaltyInfo = {...}` here would only live in game_store's sandbox.
    _G.LoyaltyInfo = {
    ...
    if modules.game_skills and modules.game_skills.refresh then
...
function init()
  ...
  ProtocolGame.registerExtendedJSONOpcode(STORE_EXTENDED_OPCODE, onStoreExtendedJSONOpcode)
  -- Character auction binario (header 0x80 do servidor).
  ProtocolGame.registerOpcode(Bazaar.OPCODE_RECV, Bazaar.onAuctionOpcode)
end
...
function terminate()
  ...
  offerCheckBox = nil
  ProtocolGame.unregisterExtendedJSONOpcode(STORE_EXTENDED_OPCODE, onStoreExtendedJSONOpcode)
  ProtocolGame.unregisterOpcode(Bazaar.OPCODE_RECV)
...
function onEnterSearch()
  ...
  Store:requestOffers(OPEN_SEARCH, text, 0)
end
...
function openBaazarWindow()
  ...
  -- Forca o estado inicial (passo 1/3 = regras) ANTES da resposta do servidor.
  -- Sem isto, a janela reabria no painel/passo da sessao anterior (ex: 2/3),
  -- em vez de respeitar a sequencia 1 -> 2 -> 3. O texto de regras e os
  -- requeriments sao repopulados quando o RECV_MAIN chega (onCharacterBazarRequeriments).
  Bazaar.step = 0
  Bazaar.requeriments = Bazaar.requeriments or {}
  Bazaar:showRules()
  -- Abre o character auction em binario (recvbyte 0x76, subtipo 0). O servidor
  -- responde com MainWindow + Depot + Store + Arguments (header 0x80).
  Bazaar.sendOpen()
end
...
function onCharacterBazarCheckInformations(initialFee, errors)
  Bazaar.initialFee = initialFee
  -- Reavalia o botao de preco com a taxa real recebida do servidor.
  if bazaarWindow and bazaarWindow:isVisible() then
    local priceInput = bazaarWindow:recursiveGetChildById('priceInputBox')
    if priceInput then
      Bazaar:checkPriceInput(priceInput:getText())
    end
  end
  -- Erros de validacao (FeeAndTexts). Mostra ao jogador antes de confirmar.
  if type(errors) == "table" and #errors > 0 then
    local message = table.concat(errors, "\n")
    if displayErrorBox then
      displayErrorBox(tr('Character Auction'), message)
    else
      g_logger.warning("[Bazaar] " .. message)
    end
  end
end
