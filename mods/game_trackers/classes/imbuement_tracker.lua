...
local INVENTORY_SLOT_PURSE = 11

local function normalizeSlots(data)
	local slots = {}
	local totalSlots = tonumber(data.totalSlots or #data.slots or 0) or 0
	for i = 1, totalSlots do
		local slot = data.slots and data.slots[i] or nil
		slots[i] = {
			imbuementId = tonumber(slot and (slot.imbuementId or slot.iconId) or 0) or 0,
			time = tonumber(slot and (slot.time or slot.duration) or 0) or 0,
			description = slot and (slot.description or slot.name) or ""
		}
	end
	return slots
end
...
function ImbuementTracker.showTrackerData()
	if not imbuementData or not g_game.isOnline() or not imbuementTrackerWindow then
		return
	end
	...
	for _, data in pairs(imbuementData) do
		if tonumber(data.slotPosition or data.slot or 0) ~= INVENTORY_SLOT_PURSE then
			data.slots = normalizeSlots(data)
			local canShow = true
			local emptySlots = 0
			for k, v in ipairs(data.slots) do
	...
	for _, data in pairs(sortedData) do
		...
		-- Imbuement Tracker shows the same item over and over (one row per
		-- imbued piece) and the rarity gold/silver/etc frame just makes the
		-- list noisy and harder to scan. Mark the slot so ItemsDatabase
		-- skips drawing the rarity frame (see gamelib/items.lua).
		widget.itemSlot._skipRarityFrame = true
		...
		for k, v in ipairs(data.slots) do
			...
			if source and v.imbuementId ~= 0 then
