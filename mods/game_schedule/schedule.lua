...
function toggle(eventTime)
  if scheduleWindow:isVisible() then
    ...
    -- Abre no dia certo: se um evento foi clicado, usa a data dele (ATIVO=hoje, UPCOMING=startdate);
    -- senao (botao do schedule) abre no mes ATUAL lendo o dia de hoje. Antes abria no evento mais
    -- ANTIGO da lista, que podia ser um evento passado (ex: Marco) e abria no mes errado.
    -- 'step' fica sincronizado para next/prevMonth navegarem a partir de hoje.
    local target = type(eventTime) == 'number' and eventTime or os.time()
    local cur = os.date("*t")
    local ev = os.date("*t", target)
    step = (ev.year * 12 + ev.month) - (cur.year * 12 + cur.month)
    makeCalendar(target)

    -- configure event (reforca com os eventos atuais)
