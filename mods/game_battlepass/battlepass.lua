...
-- Opcode binario nativo (substitui o extended/JSON 204). Deve casar com
-- Proto::GameServerBattlePass no servidor (protocolgame.cpp send/parse).
local BATTLEPASS_OPCODE = 188

local function getMissionIndex(index)
    return MissionsDisplacement[index]
end

-- Le uma lista de missoes no formato binario do servidor.
local function readMissions(msg)
    local missions = {}
...
-- Handler do opcode binario 0xBC. subType: 0=missions, 1=rewards.
local function onBattlePassOpcode(protocol, msg)
    ...
    if subType == 0 then
        ...
        local beginTime = msg:getU32()
        local endTime = msg:getU32()
        local points = msg:getU32()
        local rerollPrice = msg:getU16()
        local battlePassActive = msg:getU8() == 1
        local currentRewardStep = msg:getU16()
        local nextStepPoints = msg:getU32()
        local dailyBeginTime = msg:getU32()
        local dailyEndTime = msg:getU32()
        local dailyMissions = readMissions(msg)
        local generalMissions = readMissions(msg)

        -- [debug log desativado] g_logger.info(string.format(
        --     "[BattlePass] missions points=%d rewardStep=%d daily=%d general=%d active=%s",
        --     points, currentRewardStep, #dailyMissions, #generalMissions, tostring(battlePassActive)))

        g_game.onBattlePassMissions(playerOutfit, beginTime, endTime, points, rerollPrice,
            battlePassActive, currentRewardStep, nextStepPoints, dailyBeginTime, dailyEndTime,
            dailyMissions, generalMissions)

    elseif subType == 1 then
        local rewardSteps = {}
        local stepCount = msg:getU8()
        for _ = 1, stepCount do
            local step = { stepId = msg:getU16(), rewards = {} }
            local rewardCount = msg:getU8()
            for _ = 1, rewardCount do
                step.rewards[#step.rewards + 1] = {
                    rewardId = msg:getU16(),
                    rewardType = msg:getU8(),
                    itemId = msg:getU32(),
                    count = msg:getU16(),
                    charges = msg:getU16(),
                    freeReward = msg:getU8() == 1,
                    hasClamedReward = msg:getU8() == 1,
                    stuck = msg:getU8() == 1,
                }
            end
            rewardSteps[#rewardSteps + 1] = step
        end

        -- [debug log desativado] g_logger.info(string.format("[BattlePass] rewards steps=%d", #rewardSteps))
        g_game.onBattlePassRewards(rewardSteps)
    else
        g_logger.warning("[BattlePass] unknown opcode subType: " .. tostring(subType))
    end
end
...
function redirectToStore()
	...
	g_game.openStore()
	g_game.sendRequestUsefulThings(20)
end
...
function init()
    ...
    ProtocolGame.registerOpcode(BATTLEPASS_OPCODE, onBattlePassOpcode)
end
...
function terminate()
    ...
    ProtocolGame.unregisterOpcode(BATTLEPASS_OPCODE)
end
...
function BattlePass:showBattlePass()
    ...
    -- Pede o saldo atual ao servidor; a resposta dispara onResourceBalance e mantem o gold em dia.
...
function show()
    ...
    -- Pede o saldo ao servidor e popula o gold ao abrir (window ja visivel acima). A resposta
    -- do servidor dispara onResourceBalance de novo, mantendo o valor sempre atualizado.
    g_game.requestResource(ResourceBank)
    g_game.requestResource(ResourceInventary)
    onResourceBalance()
end
...
function loadMenu(menuId)
    if g_game.isOnline() and not BattlePass.window:isVisible() then
    ...
    elseif menuId == 'rewardsMenu' then
        ...
        if g_game.requestBattlePass then
...
function BattlePass.calculateWeekNumber()
    if not BattlePass.beginTime then return 1 end
    local targetTime = os.time()
    local t = os.date("*t", BattlePass.beginTime)
    if not t then return 1 end
    local begindate = os.time{year=t.year, month=t.month, day=t.day, hour=10, min=0, sec=0}
    if not begindate then return 1 end
...
function BattlePass.getNextResetWeek(currentIndex)
    if not BattlePass.beginTime then return 0 end
    local nextDays = 7 * currentIndex
    local t = os.date("*t", BattlePass.beginTime)
    if not t then return 0 end
    local begindate = os.time{year=t.year, month=t.month, day=t.day, hour=10, min=0, sec=0}
    if not begindate then return 0 end
...
function BattlePass:rerollDailyMission(data)
    ...
    local okButton = function()
        ...
        BattlePass.dailyRerollWindow = nil
        g_client.setInputLockWidget(nil)
        if g_game.rerollBattlePassMission then
