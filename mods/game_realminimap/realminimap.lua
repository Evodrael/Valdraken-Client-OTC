...
function init()
  ...
  minimapButton = modules.client_topmenu.addRightGameToggleButton('realMinimapButton',
...
function updateCameraPosition()
  ...
  if not minimapWidget:isDragging() and not fullmapView then
...
function loadMap()
  ...
  local player = g_game.getLocalPlayer()
  local pos = player and player:getPosition() or {x = 31000, y = 31000, z = 7}

  minimapWidget:setCameraPosition(pos)
  minimapWidget:setCrossPosition(pos)
  minimapWidget.view = (g_satelliteMap and g_satelliteMap.hasRealMapChunksForFloor and g_satelliteMap.hasRealMapChunksForFloor(pos.z)) and "realmap" or "satellite"
  minimapWidget:setCurrentView(minimapWidget.view)

  minimapWidget.onFloorChange = function(self, newPos, oldPos)
    if g_satelliteMap and g_satelliteMap.hasRealMapChunksForFloor and g_satelliteMap.hasRealMapChunksForFloor(newPos.z) then
      minimapWidget.view = "realmap"
      minimapWidget:setCurrentView("realmap")
      minimapWidget:setBackgroundColor("#000000ff")
    elseif newPos.z > 7 then
      minimapWidget:setCurrentView("minimap")
      minimapWidget:setBackgroundColor("#000000ff")
    else
      minimapWidget.view = "satellite"
      minimapWidget:setCurrentView("satellite")
      minimapWidget:setBackgroundColor("#274DA6")
    end
  end

  if RealMap and RealMap.setRegion then
    RealMap.setRegion(minimapWidget)
  end
  if RealMap and RealMap.setUIMarkers then
    RealMap.setUIMarkers(minimapWidget, pos, 120)
  end

  if false then

  -- g_realMinimap:clean()
  if not g_realMinimap or not g_realMinimap.loadImage or not g_realMinimap.loadRegion then
    consoleln("Real Minimap backend is unavailable.")
    return
  end
