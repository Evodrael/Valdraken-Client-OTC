...
function toggleIcon(bitChanged)
  ...
  if icon then
    icon:destroy()
    -- Note: there used to be `player:lockWalk(2000)` here when the Paralyze icon
    -- was removed. That code is the source of the reported "2 second freeze":
    -- whenever paralyze ended (timed out, was overwritten by a new paralyze,
    -- or stripped by PZ entry) the client locked walking for 2s — the opposite
    -- of what's expected (paralyze ending should free the player, not lock it
    -- further). Removed: paralyze pacing is owned by the server's stepDuration,
    -- not by a client-side cooldown on icon removal.
  else
...
function onManaChange(localPlayer, mana, maxMana)
  ...
  local manaLabel = healthInfoWindow and healthInfoWindow:recursiveGetChildById("manaLabel")
  if manaLabel then manaLabel:setText(mana) end
...
local function formatCapShort(value)
  value = math.floor(tonumber(value) or 0)
  -- Keep the full integer up to 99999 then abbreviate to "100k"+ / "1M"+.
  -- Mirrors game_inventory.formatCapacityShort so the two HUD widgets show
...
function onFreeCapacityChange(player, freeCapacity)
  capLabel:setText(tr('Cap') .. ': ' .. formatCapShort(freeCapacity))
end
