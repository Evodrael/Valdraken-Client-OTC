...
-- When the Luminaris-style NPC dialog is loaded AND open, the upstream
-- behavior expects the trade window to dock INSIDE the dialog (right of the
-- chat history) instead of in the left/right interface panel. This function
-- returns the dock target widget if that condition is met, nil otherwise.
local function getNpcDialogTradeDock()
  local dlg = modules.npc_dialog
  if not dlg or not dlg.getWindow or not dlg.getTradeContainer then return nil end
  local win = dlg.getWindow()
  if not win or not win:isVisible() then return nil end
  local container = dlg.getTradeContainer()
  if not container then return nil end
  return container
end

-- 15.24 "Sell All" (Lootmonger) shortcut button rendered next to the keyword
-- buttons inside the npc_dialog. Mirrors the upstream Luminaris behavior.
function removeLootmongerSellButton()
  if lootmongerSellButton and not lootmongerSellButton:isDestroyed() then
    lootmongerSellButton:destroy()
  end
  lootmongerSellButton = nil
end

function addLootmongerSellButton()
  removeLootmongerSellButton()
  local dlg = modules.npc_dialog
  if not dlg or not dlg.getWindow then return end
  local npcDialog = dlg.getWindow()
  if not npcDialog then return end
  local buttonContainer = npcDialog:recursiveGetChildById('buttonContainer')
  if not buttonContainer then return end

  lootmongerSellButton = g_ui.createWidget('Button', buttonContainer)
  lootmongerSellButton:setId('buttonSellAll')
  lootmongerSellButton:setWidth(34)
  lootmongerSellButton:setHeight(34)
  lootmongerSellButton:setTooltip('Sell All')
  lootmongerSellButton.onClick = function()
    -- onTradeAllClick is the OTCV8 Quick Sell entry point — it opens the
    -- selection window where the player deselects items they want to keep
    -- and then sells the rest. Same UX as the upstream Sell All.
    if onTradeAllClick then onTradeAllClick() end
  end

  local icon = g_ui.createWidget('UIWidget', lootmongerSellButton)
  icon:setId('icon')
  icon:setWidth(32)
  icon:setHeight(32)
  ...
  icon:setImageSource('/modules/game_npctrade/images/sellallicon')
...
function show()
  if g_game.isOnline() then
    ...
    -- Prefer docking inside the npc_dialog if it's open (15.24 layout). Falls
    -- back to the regular left/right panel placement when the dialog isn't
    -- around (e.g. /buy from the chat with no dialog open).
    local tradeDock = getNpcDialogTradeDock()
    if tradeDock then
      if npcWindow:getParent() ~= tradeDock then
        npcWindow:setParent(tradeDock)
      end
      npcWindow:fill('parent')
      -- Lock the window in place so the player can't accidentally drag it
      -- outside the dialog (and so the title bar doesn't compete with the
      -- dialog drag). Also hide the close 'X' since the dialog owns that.
      if npcWindow.setDraggable then npcWindow:setDraggable(false) end
      local closeButton = npcWindow:recursiveGetChildById('closeButton')
      if closeButton then closeButton:setVisible(false) end
      local miniwindowClose = npcWindow:recursiveGetChildById('miniwindowCloseButton')
      if miniwindowClose then miniwindowClose:setVisible(false) end
      if modules.npc_dialog.expandForTrade then
        modules.npc_dialog.expandForTrade()
      end
      npcWindow:show()
      npcWindow.close = function() closeNpcTrade() end
      npcWindow:focus()
      setupPanel:enable()
      -- Add the 15.24 "Sell All" button next to the keyword buttons inside
      -- the dialog so the player can dump loot without scrolling the list.
      addLootmongerSellButton()
      return
    end
...
function hide()
  ...
  -- If the window was docked inside the npc_dialog (15.24 layout), reparent
  -- back to the default container panel before hiding so the next regular
  -- /buy /sell with no dialog still places it in the left/right panel.
  local dockedInDialog = npcWindow:getParent() and npcWindow:getParent():getId() == 'tradeContainer'
  if dockedInDialog and m_interface and m_interface.getContainerPanel then
    npcWindow:setParent(m_interface.getContainerPanel())
    -- Clear fill-parent constraint so the window has its native size again.
    npcWindow:breakAnchors()
    -- Restore drag and the close button so a future standalone open behaves.
    if npcWindow.setDraggable then npcWindow:setDraggable(true) end
    local closeButton = npcWindow:recursiveGetChildById('closeButton')
    if closeButton then closeButton:setVisible(true) end
    local miniwindowClose = npcWindow:recursiveGetChildById('miniwindowCloseButton')
    if miniwindowClose then miniwindowClose:setVisible(true) end
  end

  -- Remove the dialog-side Sell All button if it was added on show.
  removeLootmongerSellButton()
...
-- Async batch refresh — splits widget creation into 30-item chunks across
-- frames so large shops (200+ items) don't freeze the client. The widget
-- holding everything is the same `itemsPanel`. If a new refresh starts
-- before the previous finished, the in-flight token is invalidated and we
...
local function buildItemBox(item)
  ...
  if radioItems then radioItems:addWidget(itemBox) end
end

local function refreshTradeItemsAsync()
  if not g_game.isOnline() then return end

  -- Cancel any pending batch from a previous refresh.
  ...
  local index = 1
  local total = #currentTradeItems

  local function processBatch()
    if refreshToken ~= token then return end -- a newer refresh superseded us
    local stop = math.min(index + ITEMS_PER_BATCH - 1, total)
    for i = index, stop do
      local item = currentTradeItems[i]
      if not (tradeType == SELL and not canTradeItem(item)) then
        buildItemBox(item)
      end
    end
    index = stop + 1
...
function refreshTradeItems()
  -- Throttle re-entrancy: if multiple callers fire refresh in the same tick
  -- (e.g. inventory updates while opening the shop), coalesce into one run.
  if refreshThrottleEvent then return end
  refreshThrottleEvent = scheduleEvent(function()
    refreshThrottleEvent = nil
    refreshTradeItemsAsync()
  end, 0)
end
...
-- Quick Sell tracker: accumulates gold from server "Sold X for Y gold."
-- messages during a sellAll burst and reports the total when the burst ends.
local quickSellActive = false
local quickSellAccumulatedGold = 0
local quickSellItemCount = 0
local quickSellFlushEvent = nil

local function quickSellOnTextMessage(mode, msg)
  if not quickSellActive or mode ~= MessageModes.TradeNpc then return end
  local count = msg:match("Sold (%d+)")
  local gold = msg:match("for (%d+) gold")
  if gold then
    quickSellAccumulatedGold = quickSellAccumulatedGold + (tonumber(gold) or 0)
    if count then
      quickSellItemCount = quickSellItemCount + (tonumber(count) or 0)
    end
  end
end

local function quickSellFlush()
  if quickSellAccumulatedGold > 0 then
    local human = (quickSellItemCount > 0 and (quickSellItemCount .. " items for ") or "")
                  .. comma_value(quickSellAccumulatedGold) .. " gold"
    if modules.game_textmessage and modules.game_textmessage.displayStatusMessage then
      modules.game_textmessage.displayStatusMessage("Quick Sell: " .. human)
    end
    if modules.game_console and modules.game_console.addText then
      modules.game_console.addText("Quick Sell total: " .. human,
        modules.game_console.SpeakTypesSettings, tr("Server Log"))
    end
  end
  quickSellActive = false
  quickSellAccumulatedGold = 0
  quickSellItemCount = 0
  quickSellFlushEvent = nil
end

local function quickSellArm()
  if not quickSellActive then
    quickSellActive = true
    quickSellAccumulatedGold = 0
    quickSellItemCount = 0
    connect(g_game, { onTextMessage = quickSellOnTextMessage })
  end
  -- Sliding window: every new sellItem extends the flush deadline by 800 ms.
  if quickSellFlushEvent then removeEvent(quickSellFlushEvent) end
  quickSellFlushEvent = scheduleEvent(function()
    disconnect(g_game, { onTextMessage = quickSellOnTextMessage })
    quickSellFlush()
  end, 800)
end
...
function getPlayerMoney()
  ...
  if CURRENCYID and CURRENCYID ~= GOLD_COINS and CURRENCYID > 0 then
