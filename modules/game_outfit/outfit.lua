...
function onShowFloorChange(checkBox, checked)
  if checked then
		...
		window.preview.previewoutfit:setImageSource('/mods/game_cyclopedia/images/ui/panel-background')
	end
end
...
local function normalizeCurrentOutfit(currentOutfit)
  currentOutfit = currentOutfit or {}
  currentOutfit.addons = tonumber(currentOutfit.addons or 0) or 0
  currentOutfit.mount = tonumber(currentOutfit.mount or 0) or 0
  currentOutfit.familiar = tonumber(currentOutfit.familiar or 0) or 0
  currentOutfit.aura = tonumber(currentOutfit.aura or currentOutfit.auras or 0) or 0
  currentOutfit.auraId = tonumber(currentOutfit.auraId or currentOutfit.aura or 0) or 0
  currentOutfit.auraCategory = tonumber(currentOutfit.auraCategory or 0) or 0
  currentOutfit.wing = tonumber(currentOutfit.wing or currentOutfit.wings or 0) or 0
  currentOutfit.effect = tonumber(currentOutfit.effect or currentOutfit.effects or 0) or 0
  currentOutfit.shader = currentOutfit.shader or currentOutfit.shaders or ""
  currentOutfit.head = tonumber(currentOutfit.head or 0) or 0
  currentOutfit.body = tonumber(currentOutfit.body or 0) or 0
  currentOutfit.legs = tonumber(currentOutfit.legs or 0) or 0
  currentOutfit.feet = tonumber(currentOutfit.feet or 0) or 0
  currentOutfit.mountHead = tonumber(currentOutfit.mountHead or currentOutfit.head or 0) or 0
  currentOutfit.mountBody = tonumber(currentOutfit.mountBody or currentOutfit.body or 0) or 0
  currentOutfit.mountLegs = tonumber(currentOutfit.mountLegs or currentOutfit.legs or 0) or 0
  currentOutfit.mountFeet = tonumber(currentOutfit.mountFeet or currentOutfit.feet or 0) or 0
  return currentOutfit
end

local function isColoredMountWidget(widget)
  if not widget or not widget.isColoredMount then
    return true
  end

  return widget:isColoredMount()
end

local function getOutfitMode(data)
  return tonumber(data and data[4] or 0) or 0
end

local function getOutfitStoreOffer(data)
  return tonumber(data and data[5] or 0) or 0
end

local function getMountMode(data)
  return tonumber(data and data[3] or 0) or 0
end

local function getMountStoreOffer(data)
  return tonumber(data and data[4] or 0) or 0
end

local function openStoreAppearance(offerId, searchText)
  window:hide()
  g_game.openStore()

  if offerId and offerId > 0 then
    g_game.sendRequestStoreOfferById(offerId, 0, 0)
  elseif searchText and #searchText > 0 then
    g_game.sendRequestStoreSearch(searchText, 0, 0)
  end
end
...
function create(currentOutfit, outfitList, mountList, familiarList, tryType, mount, randomMount, auraList)
  ...
  loadSettings()
  currentOutfit = normalizeCurrentOutfit(currentOutfit)
  outfitList = outfitList or {}
  mountList = mountList or {}
  familiarList = familiarList or {}
  auraList = auraList or {}

  -- game_hireling_outfit: tryType == 3 indica janela de outfit de hireling. O creatureId
  -- do hireling foi guardado no Game pelo parse (parseOpenOutfitWindow). Resetado a cada abertura.
  hirelingCreatureId = (tryType == 3 and g_game.getOutfitCreatureId) and g_game.getOutfitCreatureId() or 0

  -- Protocol sends {id, name}; visual aura id == id (index [1])
  for i = 1, #auraList do
    local auraData = auraList[i]
    if auraData[1] == currentOutfit.aura then
  ...
  if not table.empty(ServerData.auras) then
    if currentOutfit.auraId == 0 then
      ...
      -- Protocol sends {id, name}: id=data[1], name=data[2], category=0 (not in protocol)
      currentOutfit.aura = data[1]
      currentOutfit.auraCategory = 0
      currentOutfit.auraId = data[1]
      updateAppearanceText("aura", data[2])
    end
  end
end
...
function showOutfits(searchText)
  ...
  for _, outfitData in ipairs(availableOutfits) do
    ...
    local mode = getOutfitMode(outfitData)
    local storeOffer = getOutfitStoreOffer(outfitData)
    if mode == 1 or storeOffer > 0 then
        button:setImageSource("/images/ui/large_blue_button")
        button:setActionId(storeOffer)
        button.storeOfferId = storeOffer
        button.storeSearchText = outfitData[2]
    end
...
function showMounts(searchText)
  ...
  for _, mountData in ipairs(availableMounts) do
    ...
    local mode = getMountMode(mountData)
    local storeOffer = getMountStoreOffer(mountData)
    if mode == 1 or storeOffer > 0 then
        button:setImageSource("/images/ui/large_blue_button")
        button:setActionId(storeOffer)
        button.storeOfferId = storeOffer
        button.storeSearchText = mountData[2]
    end

    if tempOutfit.mount == mountData[1] then
      focused = mountData[1]
      if not isColoredMountWidget(button.outfit) then
...
function showAuras()
  ...
  for _, auraData in ipairs(ServerData.auras) do
    -- Protocol sends {id, name}: id=auraData[1], name=auraData[2]
    local button = g_ui.createWidget("SelectionButton", window.ScrollBar.selectionList)
    button:setId(auraData[1])

    button.aura = auraData[1]
    button.auraCategory = 0

    local outfit = table.copy(previewCreature:getOutfit())
    outfit.aura = auraData[1]
    outfit.auraCategory = 0
    button.outfit:setOutfit(outfit)
    button.outfit:setCenter(true)
  ...
  if focused ~= nil then
    ...
    if not table.empty(ServerData.auras) then
      if tempOutfit.aura == 0 then
        updateAppearanceText("aura", ServerData.auras[1][2])
...
function onPresetSelect(widget)
  ...
  if storeMount > 0 then
    ...
    window.appearance.mount.onClick = function() openStoreAppearance(storeMount, window.appearance.mount.name:getText()) end
  else
  ...
  if storeOutfit > 0 then
    ...
    window.appearance.outfit.onClick = function() openStoreAppearance(storeOutfit, window.appearance.outfit.name:getText()) end
  else
...
function onOutfitSelect(list, focusedChild, unfocusedChild, reason)
  if focusedChild then
    ...
    local storeOffer = focusedChild.storeOfferId or focusedChild:getActionId()
		if storeOffer > 0 or focusedChild.storeSearchText then
			window.appearance.outfit:setImageSource("/images/ui/hlarge-blue-button")
      window.appearance.outfit.purse:setVisible(true)
      window.appearance.outfit.onClick = function() openStoreAppearance(storeOffer, focusedChild.storeSearchText or focusedChild.name:getText()) end
      window.okButton:setEnabled(false)
		else
			window.appearance.outfit:setImageSource("/images/ui/pressed-large-button")
      window.appearance.outfit.purse:setVisible(false)
  ...
  if focusedChild then
    ...
    local storeOffer = focusedChild.storeOfferId or focusedChild:getActionId()
		if storeOffer > 0 or focusedChild.storeSearchText then
			window.appearance.mount:setImageSource("/images/ui/hlarge-blue-button")
      window.appearance.mount.purse:setVisible(true)
      window.appearance.mount.onClick = function() openStoreAppearance(storeOffer, focusedChild.storeSearchText or focusedChild.name:getText()) end
      window.okButton:setEnabled(false)
		else
			window.appearance.mount:setImageSource("/images/ui/pressed-large-button")
      window.appearance.mount.purse:setVisible(false)
    ...
    window.appearance.grayHover:setVisible(false)
    if not isColoredMountWidget(focusedChild.outfit) then
      window.appearance.grayHover:setVisible(true)
    end
  end
end
  ...
  for key, value in pairs(outfit) do
    ...
    if dataTable then
      for _, data in ipairs(dataTable) do
        if outfit[key] == data[1] then
          updateAppearanceText(appKey, data[2] or "")
        end
      end
    end
  end
...
function getStoreMount(mountId)
  for _, data in pairs(ServerData.mounts) do
    if getMountMode(data) ~= 0 and data[1] == mountId then
      return getMountStoreOffer(data)
    end
  end
  return 0
end
...
function getStoreOutfit(outfitId)
  for _, data in pairs(ServerData.outfits) do
    if getOutfitMode(data) ~= 0 and data[1] == outfitId then
      return getOutfitStoreOffer(data)
    end
  end
  return 0
end
...
function accept()
  ...
  -- Hireling: aplica via changeHirelingOutfit (outfitType 1 + creatureId). Caso contrario,
  -- outfit normal do jogador.
  if hirelingCreatureId and hirelingCreatureId > 0 and g_game.changeHirelingOutfit then
    g_game.changeHirelingOutfit(tempOutfit, hirelingCreatureId)
  else
