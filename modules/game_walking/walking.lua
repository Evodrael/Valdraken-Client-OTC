...
-- Note: paralyze pacing is owned by the C++ side and matches OTC exactly.
-- The server delivers the real reduced speed via SpeedChange; Creature::getStepDuration
-- returns `1000 * groundSpeed / m_speed` (or 0 when m_speed<1, i.e. full paralyze).
-- canWalk() naturally throttles input to that pace, and the existing walk animation
-- engine paints a smooth slow-walk because each frame advances m_walkedPixels in
-- proportion to the step duration. There is intentionally NO paralyze bypass and NO
-- artificial speed floor — both produced freezes (the floor desynced client step
-- pacing from server pacing, so the server kept rejecting walks → cancelWalk →
-- lockWalk(50), which is what visibly froze the character).
...
function walk(dir, ticks)
  ...
  if player:isWalking() and player:getPreWalkLockedDelay() >= g_clock.millis() then
    return
  end
  ...
  if player and not player:canWalk() then -- canWalk returns false when previous walk is not finished or not confirmed by server
    -- Match OTC: no paralyze-specific bypass. The protocol-level speed floor at
    -- protocolgameparse.cpp:2462 (speed = max(speed, 80) for the local player) keeps
    -- getStepDuration finite, so canWalk() correctly throttles steps to the paralyzed
    -- pace. The previous bypass kept spamming walk packets that the server rejected
    -- with cancelWalk → lockWalk, which is what visibly froze the character.
    if dash then
      ignoredCanWalk = true
    else
      if ticksToNextWalk < 2500 and (lastWalkDir ~= dir or ticks == 0 or player:isParalyzed()) then
        nextWalkDir = dir
      end
  ...
  if toTile and toTile:isWalkable() and not toTile:hasItem(ITEM_WILD_GROWTH) then
    if not player:isServerWalking() and not ignoredCanWalk then
      ...
      if not table.contains(diagonalDirs, dir) and (player:getPreWalkLockedDelay() < g_clock.millis() and not table.contains(diagonalDirs, lastWalkDir)) then
        player:preWalk(dir)
        preWalked = true
      end
    end
