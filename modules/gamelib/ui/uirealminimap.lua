...
function UIRealMinimap:onMouseRelease(pos, button)
  ...
  if widgetInfo and widgetInfo.type == "city" then
    ...
    local regions = g_things.getSubAreaById and g_things.getSubAreaById(widgetInfo.widgetId) or {}
  ...
  if customMouseEvents then
    for _, customMouseEvent in ipairs(customMouseEvents) do
      if mapPos.x >= customMouseEvent.fromMapPos.x and
        ...
        if customMouseEvent.callback(self, mapPos, pos) then
          return true
        end
      end
    end
...
local _realMinimapWidgetIdCounter = 0

function UIRealMinimap:addWidget(imagePath, imageSize, pos, tooltip)
  _realMinimapWidgetIdCounter = _realMinimapWidgetIdCounter + 1
  local id = _realMinimapWidgetIdCounter
...
function UIRealMinimap:getWidgetInfoFromPoint(screenPoint)
...
function UIRealMinimap:setCurrentView(view)
  self.currentViewMode = view
  local isRealMap = view == "realmap" or view == "ingame"
  if self.setRealMapMode then
    self:setRealMapMode(isRealMap)
  end
  self:setUseStaticMinimap(not isRealMap)
  self:setSatelliteMode(view == "satellite")
end

function UIRealMinimap:setupHouse(houseId)
  self.selectedHouse = houseId or 0
  self.housePosition = nil

  if self.selectedHouse == 0 then
    return
  end

  local houseList = g_things.getHouseList and g_things.getHouseList() or {}
  local house = houseList[self.selectedHouse]
  if house and house.position then
    self.housePosition = house.position
  elseif HOUSE and HOUSE[self.selectedHouse] then
    local data = HOUSE[self.selectedHouse]
    if data.entryx and data.entryy and data.entryz then
      self.housePosition = {x = data.entryx, y = data.entryy, z = data.entryz}
    end
  end
end

function UIRealMinimap:setSelectedCity(cityId)
  self.selectedCity = cityId or 0
end

function UIRealMinimap:getAreaNameById(areaId)
  local label = tostring(areaId or 0)
  return "Area " .. label, "Subarea " .. label
end
