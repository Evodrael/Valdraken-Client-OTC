...
function UIMinimap:removeAlternative(widget)
  for _, alternative in pairs(self.alternatives) do
    if alternative == widget then
      table.removevalue(self.alternatives, alternative)
      break
    end
  end
end
...
function UIMinimap:showParty()
  local partyMode = self.partyMode or self:recursiveGetChildById('partyMode')
  if not partyMode then
    return
  end

  self.partyMode = partyMode
  if not partyMode:isVisible() then
    partyMode:show()
  end
end
...
local _minimapWidgetIdCounter = 0

function UIMinimap:addWidget(imagePath, imageSize, pos, tooltip)
  _minimapWidgetIdCounter = _minimapWidgetIdCounter + 1
  local id = _minimapWidgetIdCounter
  if imagePath and imagePath:sub(1,1) ~= '/' then imagePath = '/' .. imagePath end

  local widget = g_ui.createWidget('UIWidget', self)
  widget:setImageSource(imagePath)
  widget:setWidth(imageSize.width)
  widget:setHeight(imageSize.height)
  widget:setPhantom(true)
  widget:lower()

  self:centerInPosition(widget, pos)

  widget.minimapPos      = pos
  widget.minimapTooltip  = tooltip
  widget.minimapImgPath  = imagePath
  widget.minimapWidgetId = id

  if not self.minimapWidgets then
    self.minimapWidgets = {}
  end
  self.minimapWidgets[id] = widget
  return id
end

function UIMinimap:removeWidget(id)
  if not self.minimapWidgets then return end
  local widget = self.minimapWidgets[id]
  if widget then
    widget:destroy()
    self.minimapWidgets[id] = nil
  end
end

function UIMinimap:moveWidget(id, pos)
  if not self.minimapWidgets then return end
  local widget = self.minimapWidgets[id]
  if widget then
    widget.minimapPos = pos
...
function UIMinimap:getWidgetInfoFromPoint(screenPoint)
  if not self.minimapWidgets then return nil end
  local hitRadius = 8
  for id, widget in pairs(self.minimapWidgets) do
    local tp = self:getTilePoint(widget.minimapPos)
    if tp and tp.x >= 0 then
      if math.abs(screenPoint.x - tp.x) <= hitRadius and
         math.abs(screenPoint.y - tp.y) <= hitRadius then
        return {
          type              = "flag",
          pos               = widget.minimapPos,
          tooltip           = widget.minimapTooltip,
          imagePath         = widget.minimapImgPath,
          widgetId          = id,
...
function UIMinimap:clearWaypoints()
  self.waypoints = nil
  if self.clearWaypointsNative then
    self:clearWaypointsNative()
  end
end

function UIMinimap:setDrawWaypoints(enabled)
  self.drawWaypoints = enabled
  if self.setDrawWaypointsNative then
    self:setDrawWaypointsNative(enabled and true or false)
  end
end

function UIMinimap:makeWaypoints(coordinates, floor)
  if not self.waypoints then self.waypoints = {} end
  self.waypoints[floor] = coordinates
  if self.makeWaypointsNative then
    self:makeWaypointsNative(coordinates, tonumber(floor) or -1)
  end
end

function UIMinimap:clearRoutePath()
  self.routePath = nil
  if self.clearRoutePathNative then
    self:clearRoutePathNative()
  end
end

function UIMinimap:makeRouth(path, floor)
  if floor ~= nil then
    if not self.routePath then self.routePath = {} end
    self.routePath[floor] = path
  else
    self.routePath = path
  end

  if self.makeRouthNative then
    self:makeRouthNative(path, tonumber(floor) or -1)
  end
end
