...
function ProtocolLogin:parseCharacterList(msg)
  ...
  if g_game.getProtocolVersion() > 1077 then
    ...
    if rawPremDays ~= 0 and rawPremDays ~= 65535 then
      ...
      if rawPremDays > 315360000 then -- ~10y in seconds; below this can't be a Unix epoch
        account.premDays = math.floor((rawPremDays - os.time()) / 86400)
        if account.premDays < 0 then account.premDays = 0 end
      else
        account.premDays = rawPremDays
      end
    else
      account.premDays = rawPremDays
    end

    -- Server-side fallback: some TFS forks leave subStatus=0 (Free) even when the
    -- character has positive premium days. Trust the day-count as the source of truth
    -- so the character-list label doesn't lie about premium status.
    if account.subStatus == SubscriptionStatus.Free
       and (account.premDays > 0 or account.premDays == 65535) then
      account.subStatus = SubscriptionStatus.Premium
    end
  else
    account.status = AccountStatus.Ok
    account.premDays = msg:getU16()
    account.premDaysRaw = account.premDays
    account.subStatus = account.premDays > 0 and SubscriptionStatus.Premium or SubscriptionStatus.Free
  end

  -- Diagnostic for the "I'm premium but it says Free" case. Logs the exact bytes
  -- the server sent so we can tell server-side bugs apart from client misreads.
  -- [debug log desativado] g_logger.info(string.format("[CharList] status=%s subStatus=%s premDaysRaw=%s premDays=%s",
  --   tostring(account.status), tostring(account.subStatus),
  --   tostring(account.premDaysRaw), tostring(account.premDays)))
...
function ProtocolLogin:parseExtendedCharacterList(msg)
  ...
  local account = msg:getTable() or {}
  local otui = msg:getString()

  -- Mirror the non-extended path so the character-list label honours premium days
  -- even when the server fills subStatus=Free incorrectly. Some servers ship the
  -- extended account table with the premium expiration as a Unix timestamp; convert
  -- it the same way parseCharacterList does so the days reach characterlist.lua.
  if type(account) == 'table' then
    account.premDaysRaw = account.premDays
    if account.premDays and account.premDays > 315360000 then
      account.premDays = math.floor((account.premDays - os.time()) / 86400)
      if account.premDays < 0 then account.premDays = 0 end
    end
    if account.subStatus == SubscriptionStatus.Free
       and ((account.premDays or 0) > 0 or account.premDays == 65535) then
      account.subStatus = SubscriptionStatus.Premium
    end

    -- [debug log desativado] g_logger.info(string.format("[CharListExt] status=%s subStatus=%s premDaysRaw=%s premDays=%s",
    --   tostring(account.status), tostring(account.subStatus),
    --   tostring(account.premDaysRaw), tostring(account.premDays)))
  end
