...
CreatureIconCategoryQuest = 0
CreatureIconCategoryModification = 1
...
function getIconImagePath(iconId)
  ...
  elseif iconId > 0 then
    path = '/images/game/icons/quests/' .. iconId
  end
  return path
end

function getCreatureIconImagePath(iconId, category)
  if iconId == nil then
    return nil
  end

  local folder = category == CreatureIconCategoryModification and 'modifications' or 'quests'
  return '/images/game/icons/' .. folder .. '/' .. iconId
end
...
function Creature:onIconsChange(iconId, category, count)
  local imagePath = getCreatureIconImagePath(iconId, category)
  if imagePath then
    self:updateIconEffectTexture(imagePath, count or 0)
  end
end

function Creature:onIconEffectChange(icons)
  for i, icon in pairs(icons) do
    local category = icon.category
    if category == nil then
      category = icon.modification and CreatureIconCategoryModification or CreatureIconCategoryQuest
    end

    self:onIconsChange(icon.id, category, icon.count or 0)
  end
end
