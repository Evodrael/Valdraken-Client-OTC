...
function LocalPlayer:setCyclopediaMarketList(list)
  self._cyclopediaMarketList = list
end
function LocalPlayer:getCyclopediaMarketList()
  return self._cyclopediaMarketList or {}
end
function LocalPlayer:updateCyclopediaMarketList(itemId, useNpcValue)
  itemId = tonumber(itemId)
  if not itemId then
    return
  end

  local list = self:getCyclopediaMarketList()
  local itemIdString = tostring(itemId)
  local foundIndex = nil
  for index, value in pairs(list) do
    if tonumber(value) == itemId or tostring(value) == itemIdString then
      foundIndex = index
      break
    end
  end
  ...
  elseif not foundIndex then
    table.insert(list, itemId)
  end

  self._cyclopediaMarketList = list
end
function LocalPlayer:setCyclopediaCustomPrice(prices)
  self._cyclopediaCustomPrice = prices
end
function LocalPlayer:getCyclopediaCustomPrice()
  return self._cyclopediaCustomPrice or {}
end
function LocalPlayer:updateCyclopediaCustomPrice(itemId, price)
  itemId = tonumber(itemId)
  if not itemId then
    return
  end

  local prices = self:getCyclopediaCustomPrice()
  prices[itemId] = tonumber(price) or 0
  prices[tostring(itemId)] = prices[itemId]
  self._cyclopediaCustomPrice = prices
end
