    ...
    elseif mouseButton == MouseRightButton and not g_mouse.isPressed(MouseLeftButton) then
      ...
      elseif isNpc then
        menu:addOption(tr('Talk'), function()
		  ...
		  -- Prefer npc_dialog (Luminaris-style window) when present, else legacy
		  -- binding, else broadcast "hi" so the NPC's keyword handler still fires.
		  if modules.npc_dialog and modules.npc_dialog.openNpcDialogFor then
		    modules.npc_dialog.openNpcDialogFor(creature:getId(), false)
  ...
  for id, config in pairs(bCondig) do
    ...
    if (data and data:getWindow()) or data.window:isVisible() then
      ...
      for _, value in pairs(config.battleListFilters) do
        ...
        if button then
        data.panel:setFilter(invertedValue, false)
...
function chooseNextCreature()
  ...
  local creatures = UICreatureButtonList.getVisibleCreatures(getMainBattle().panel)
...
function getCreatures()
  return UICreatureButtonList.getVisibleCreatures(getMainBattle().panel)
end

function getAttackableCreatures()
  return UICreatureButtonList.getAttackableCreatures(getMainBattle().panel)
end
