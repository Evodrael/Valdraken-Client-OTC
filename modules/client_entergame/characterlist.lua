...
function CharacterList.create(characters, account, otui)
  ...
  charactersWindow.characterTable.characterSort:setTextOffset("0 0")
  ...
  -- (selecao do 1o personagem agora e garantida em CharacterList.show())
  ...
  -- Diagnostic for the "I'm premium but it shows Free Account" report. Logs whatever
  -- account fields actually reached this widget so we can tell whether the bug is in
  -- the HTTP/protocol parser, the entergame normalization, or this label's logic.
  -- [CharListUI] debug log desativado
...
function CharacterList.show()
  ...
  -- Garante que o PRIMEIRO personagem fique selecionado (com o destaque de selecao) ao abrir a
  -- lista. O charactersWindow:focus() acima rouba o foco do filho; refocamos aqui e tambem no
  -- proximo frame (addEvent) para vencer qualquer foco deferido.
  if characterList then
    local firstChar = characterList:getFirstChild()
    if firstChar then
      characterList:focusChild(firstChar, KeyboardFocusReason, true)
      characterList:ensureChildVisible(firstChar)
      -- focusChild num widget JA focado nao dispara @onFocusChange, entao o destaque visual
      -- (onUpdateOnStates) nao aplicava ate o jogador mexer (descer/subir). Aplicamos a mao.
      if onUpdateOnStates then
        onUpdateOnStates(firstChar)
      end
      addEvent(function()
        if characterList and not characterList:isDestroyed()
          and firstChar and not firstChar:isDestroyed()
          and firstChar:getParent() == characterList then
          characterList:focusChild(firstChar, KeyboardFocusReason, true)
          if onUpdateOnStates then
            onUpdateOnStates(firstChar)
          end
...
function onUpdateOnStates(self)
  ...
  if lastWidget and lastWidget ~= self then
    ...
    if lastWidget.pin then
      -- Keep the pin button visible for pinned characters even after the row
      -- loses focus, so the player can see which characters are currently pinned
      -- without having to select each one. Only hide the pin when the character
      -- isn't pinned.
      if not lastWidget.pin:isChecked() then
...
function onPinCharacter(self)
  ...
  local option = self:getParent()
  if not option or not option.name then
    return
  end

  characterList:focusChild(option, MouseFocusReason, true)
  Options.managePinnedCharacters(option.name:getText(), self:isChecked())
  onReorderCharacterList()
end
...
function onReorderCharacterList()
  ...
  for i, characterInfo in ipairs(characters) do
    ...
    -- Pin state has to mirror the persisted "pinnedCharacters" list right at widget
    -- creation, not only when the row gains focus. Otherwise the pinned icon flips
    -- to off the moment the player tabs to another character — even though the pin
    -- itself stayed pressed in Options.
    if widget.pin then
      local pinned = isCharacterPinned(characterInfo.name)
      widget.pin:setChecked(pinned)
      widget.pin:setVisible(pinned)
    end
    ...
    for key,value in pairs(characterInfo) do
      ...
      if key == 'dailyRewardState' then
        local dailyRewardState = tonumber(value)
        local source = dailyRewardState == 0 and "dailyreward_collected" or "dailyreward_notcollected"
  ...
  if focusLabel then
    -- Defer the focus so it runs *after* CharacterList.show()'s charactersWindow:focus()
    -- — otherwise that call steals focus back to the window itself and the player
    -- opens the list with no row selected. Capture in a closure since `focusLabel`
    -- is local to this function.
    local capturedFocus = focusLabel
    local capturedList = characterList
    characterList:focusChild(capturedFocus, KeyboardFocusReason, true)
    addEvent(function()
      -- Guard: by the time this fires the list may have been rebuilt (e.g. user
      -- toggled sort, or destroyAndReshow). focusChild on a stale parent throws
      -- "attempt to focus an unknown child". Re-resolve and bail if anything is
      -- destroyed or the captured widget is no longer a child.
      if not capturedList or capturedList:isDestroyed() then return end
      if not capturedFocus or capturedFocus:isDestroyed() then return end
      if capturedFocus:getParent() ~= capturedList then return end
      capturedList:focusChild(capturedFocus, KeyboardFocusReason, true)
      capturedList:ensureChildVisible(capturedFocus)
    end)
  end
end
