...
local function normalizeVipData(partyPending, groups)
  if type(partyPending) == 'table' and groups == nil then
    groups = partyPending
    partyPending = false
  end

  return partyPending or false, groups or {}
end
...
function refresh()
  ...
  for id,vip in pairs(g_game.getVips()) do
    if settings["showGrouped"] then
      ...
      partyPending, groups = normalizeVipData(partyPending, groups)
...
function createEditWindow(widget)
  ...
  local currentGroups = {}
  for _, groupId in pairs(widget.vipGroups or {}) do
  ...
  descriptionText:appendText(widget.comment or '')
  ...
  local saveFunction = function()
    ...
    local selectedIcon = iconRadioGroup:getSelectedWidget()
    local iconId = selectedIcon and tonumber(selectedIcon:getId():sub(5)) or (widget.iconId or 0)
...
function canRefreshVipGroups(id, name, state, description, iconId, notify, partyPending, groups)
  ...
  if not old or #(old.vipGroups or {}) ~= #groups then
    return true
  end
  ...
  for i = 1, #groups do
    if (old.vipGroups or {})[i] ~= groups[i] then
      return true
    end
  end
...
function showGroups()
  ...
  for _, data in pairs(vipGroups) do
    ...
    for _, k in pairs(players) do
      ...
      label.vipGroups = k.vipGroups or {}
