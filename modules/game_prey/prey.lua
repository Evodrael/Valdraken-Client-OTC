...
-- Safe wrapper for the legacy game_prey_hunting mod which has been disabled in 15.24.
-- prey.lua used to call isHuntingActive(...) inline; with
-- the mod off, those calls crash with "attempt to index field 'game_prey_hunting'".
local function isHuntingActive(monsterName)
  local mod = modules and modules.game_prey_hunting
  if mod and mod.isHuntingActive then
    return mod.isHuntingActive(monsterName)
  end
  return false
end
...
local function normalizeFreeRerollSeconds(timeleft, valueIsMinutes)
  local secondsLeft = math.max(0, tonumber(timeleft) or 0)
  if secondsLeft >= 0xF0000000 then
    return 0
  end

  if valueIsMinutes then
    secondsLeft = secondsLeft * 60
  elseif secondsLeft > os.time() then
    secondsLeft = secondsLeft - os.time()
  elseif secondsLeft > FREE_REROLL_SECONDS * 1000 then
    secondsLeft = math.floor(secondsLeft / 1000)
  end

  if secondsLeft > FREE_REROLL_SECONDS then
    secondsLeft = FREE_REROLL_SECONDS
  end

  return secondsLeft
end

local function getPreyFreeRerollStorageKey(slot)
  local player = g_game.getLocalPlayer()
  local name = player and player:getName() or "unknown"
  name = name:lower():gsub("[^%w_%-]", "_")
  return "prey_free_reroll_deadline_" .. name .. "_" .. tostring(slot or 0)
end

local function loadPreyFreeRerollDeadline(slot)
  local deadline = tonumber(g_settings.getNumber(getPreyFreeRerollStorageKey(slot), 0)) or 0
...
local function savePreyFreeRerollDeadline(slot, deadline)
  g_settings.set(getPreyFreeRerollStorageKey(slot), math.floor(tonumber(deadline) or 0))
end

local function resolveFreeRerollSeconds(slot, secondsLeft, allowReset)
  secondsLeft = math.min(math.max(0, tonumber(secondsLeft) or 0), FREE_REROLL_SECONDS)
  if secondsLeft <= 0 then
    savePreyFreeRerollDeadline(slot, 0)
    return 0
  end

  local savedDeadline = loadPreyFreeRerollDeadline(slot)
  if savedDeadline and not allowReset then
    local savedSecondsLeft = math.max(0, savedDeadline - os.time())
    if savedSecondsLeft > 0 and savedSecondsLeft < secondsLeft then
      secondsLeft = savedSecondsLeft
    end
  end

  savePreyFreeRerollDeadline(slot, os.time() + secondsLeft)
  return secondsLeft
end
...
local descriptionTable = {
  ...
  ["time"] = "You will get your next Free List Reroll in %s.\nYou get a Free List Reroll every 24 hours for each slot.",
...
function onPreyFreeRolls(slot, timeleft)
  ...
  local normalizedSeconds = normalizeFreeRerollSeconds(timeleft, true)
  local secondsLeft = resolveFreeRerollSeconds(slot, normalizedSeconds, pendingFreeRerollReset[slot])
  if normalizedSeconds > 0 then
    pendingFreeRerollReset[slot] = nil
  end
  local percent = (secondsLeft / FREE_REROLL_SECONDS) * 100
  local desc = timeleftTranslation(secondsLeft)
  ...
  for i, panel in pairs({prey.active, prey.inactive}) do
    ...
    if secondsLeft == 0 then
      ...
      price:setText(rerollPrice < 100000 and comma_value(rerollPrice) or math.ceil(rerollPrice / 1000) .. "  k")
    end
...
function setTimeUntilFreeReroll(slot, timeUntilFreeReroll) -- seconds
  local normalizedSeconds = normalizeFreeRerollSeconds(timeUntilFreeReroll, false)
  local secondsLeft = resolveFreeRerollSeconds(slot, normalizedSeconds, pendingFreeRerollReset[slot])
  if normalizedSeconds > 0 then
    pendingFreeRerollReset[slot] = nil
  end
  timeLeftRerrol[slot] = {secondsLeft = secondsLeft, startTime = os.time()}
  ...
  for i, panel in pairs({prey.active, prey.inactive, prey.select}) do
    ...
    panel.buttonsPanel.reroll.button.rerollButton.onClick = function()
      ...
      onRerollButtonAction(slot, secondsLeft <= 0)
    end
  end
end
...
function updatePreyWidget(slot, state)
  ...
  if state == SLOT_STATE_ACTIVE then
    local creatureAndBonus = preySlot.active.creatureAndBonus
    if creatureAndBonus and creatureAndBonus.creature then
  ...
  -- The legacy "Hunting Tasks" section (hslot1..3) was replaced by the
  -- 15.24 Bounty/Weekly Task slots, which are populated by the
  -- game_taskhunting bridge (modules/game_taskhunting/taskhunting_otcv8_bridge.lua).
  -- The hslot widgets are still in prey.otui (hidden) so any code that does
  -- a recursiveGetChildById('hslot1') doesn't crash; we just skip wiring
  -- callbacks here since they never become visible.
end
...
function onRerollButtonAction(slot, freeReroll)
  ...
  local okFunc = function()
    pendingFreeRerollReset[slot] = freeReroll and true or nil
...
function onPreyActive(slot, currentHolderName, currentHolderOutfit, bonusType, bonusValue, bonusGrade, timeLeft, timeUntilFreeReroll, lockType)
  ...
  prey.active.buttonsPanel.reroll.button.rerollButton.onClick = function()
    ...
    onRerollButtonAction(slot, normalizeFreeRerollSeconds(timeUntilFreeReroll, false) <= 0)
  end
...
function updateWildCardWindow()
  for i = 0, 2 do
    ...
    table.sort(currentRaces[i], function(a, b)
      ...
      local hasA = isHuntingActive(creatureA[1])
      local hasB = isHuntingActive(creatureB[1])
...
function storeRedirect(offerType)
  ...
  preyWindow:hide()
  g_game.openStore()
  g_game.sendRequestUsefulThings(offerType)
end
