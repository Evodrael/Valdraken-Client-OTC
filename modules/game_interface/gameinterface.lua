...
function init()
  ...
  connect(gameLeftPanels, { onVisibilityChange = onLeftPanelVisibilityChange })
...
function createThingMenu(tile, menuPosition, lookThing, useThing, creatureThing)
  ...
  if useThing and not useThing:isStatic() then
    ...
    if useThing:isPodium() then
      -- Qualquer podium: so envia o request (0x86). O SERVIDOR decide qual janela
      -- responder pelo tipo do item (vigour/tenacity -> 0xC2 monster; renown ->
      -- 0xC8 player). Nao dependemos mais dos client ids hardcoded (VIGOUR/TENACITY/
      -- RENOWN_PODIUM), que podiam nao bater com os ids reais do servidor -> menu
      -- "Customise Podium" aparecia mas nao fazia nada.
      menu:addOption(tr('Customise Podium'),
        function()
          g_game.requestPodiumData(useThing)
        end)
    end
  ...
  if creatureThing then
    ...
    if creatureThing:isLocalPlayer() then
      ...
      if creatureThing:isNpc() and creatureThing:getPosition().z == localPosition.z then
			...
			-- Route through npc_dialog if loaded so the Luminaris-style window
			-- appears; otherwise use the legacy server-specific binding when it
			-- exists; final fallback is a plain "hi" broadcast which any
			-- standard NPC handler picks up.
			if modules.npc_dialog and modules.npc_dialog.openNpcDialogFor then
				modules.npc_dialog.openNpcDialogFor(creatureThing:getId(), false)
			elseif g_game.sendNPCTalk then
      ...
      if creatureThing:isNpc() and creatureThing:getIcon() == NpcIconHireling then
          ...
          -- Abre a Store direto na oferta certa (categoria "Hireling Services").
          -- offer 270 = Hireling Name Change | offer 269 = Hireling Sex Change.
          menu:addOption(tr('Change Name'), function() g_game.openStore() g_game.sendRequestStoreOfferById(270, 0, 0) end, coloredText)
          menu:addOption(tr('Change Sex'), function() g_game.openStore() g_game.sendRequestStoreOfferById(269, 0, 0) end, coloredText)
          menu:addSeparator()
          -- Devolve o hireling para a lampada (volta ao Store Inbox do player).
          menu:addOption(tr('Dismiss (back to lamp)'), function() g_game.dismissHireling(creatureThing) end)
      ...
      if creatureThing:isPlayer() then
        ...
        -- Go To (so GM): teleporta ate o jogador, inclusive cross-world (reconecta).
        -- O servidor confirma a permissao; a visibilidade depende do flag de GM.
        if modules.game_announcement and modules.game_announcement.isGameMaster
            and modules.game_announcement.isGameMaster() then
          menu:addSeparator()
          menu:addOption(tr('Go To %s', creatureName), function()
            if modules.game_gamemaster and modules.game_gamemaster.gotoPlayer then
              modules.game_gamemaster.gotoPlayer(creatureName)
            end
          end)
        end
  ...
  if keyboardModifiers == KeyboardNoModifier then
    if creatureThing and mouseButton == MouseLeftButton then
      if creatureThing:isNpc() then
        ...
        if distance <= 3 then
          -- Same routing rule as the right-click Talk menu above.
          if modules.npc_dialog and modules.npc_dialog.openNpcDialogFor then
            modules.npc_dialog.openNpcDialogFor(creatureThing:getId(), false)
          elseif g_game.sendNPCTalk then
            g_game.sendNPCTalk(creatureThing:getId())
...
function processItemMenuAction(menuPosition, item)
  if not item then
    return false
  end

  createThingMenu(nil, menuPosition, item, item, nil)
  return true
end
...
function getMiniBotTimerPanel()
  return miniBotTimerPanel
end

function setMiniBotTimerPanelStatus(enabled)
  if not miniBotTimerPanel then
    return
  end

  miniBotTimerPanel:setVisible(enabled)
end
...
function showRightHorizontalPanel(visible)
  pendingHorizontalRightHeight = visible and (pendingHorizontalRightHeight > 0 and pendingHorizontalRightHeight or 200) or 0
  -- Show the dock area even when empty so dragged miniwindows have a visible
  -- drop target. The previous `visible and getChildCount() > 0` gate made the
  -- option a silent no-op for new players because the panel always starts
  -- empty, and there was no way to populate it without seeing the area.
  setHorizontalPanelState(horizontalRightPanel, 'bottomHorizontalResizeBorder', visible, pendingHorizontalRightHeight)
  setRightHorizontalWidth()
  if not visible then
...
function showLeftHorizontalPanel(visible)
  ...
  pendingHorizontalLeftHeight = visible and (pendingHorizontalLeftHeight > 0 and pendingHorizontalLeftHeight or 200) or 0
  -- Same fix as the right side: keep the dock area visible while toggled on,
  -- even with zero children, so dragging a window into it is discoverable.
  setHorizontalPanelState(horizontalLeftPanel, 'bottomHorizontalLeftResizeBorder', visible, pendingHorizontalLeftHeight)
  if not visible then
...
function setHorizontalPanelState(panel, resizeBorderId, visible, height)
  if not panel then
    return
  end

  panel:setVisible(visible)
  panel:setHeight(visible and height or 0)

  local resizeBorder = gameRootPanel and gameRootPanel:getChildById(resizeBorderId)
...
function onLoadHorizontalPanels(horizontalLeftOptions, horizontalRightOptions)
  pendingHorizontalLeftHeight = horizontalLeftOptions and horizontalLeftOptions.contentHeight or 0
  pendingHorizontalRightHeight = horizontalRightOptions and horizontalRightOptions.contentHeight or 0
  setHorizontalPanelState(horizontalLeftPanel, 'bottomHorizontalLeftResizeBorder', false, 0)
  setHorizontalPanelState(horizontalRightPanel, 'bottomHorizontalResizeBorder', false, 0)
end

function refreshHorizontalPanels()
  if horizontalLeftPanel then
    local hasContent = horizontalLeftPanel:getChildCount() > 0
    setHorizontalPanelState(horizontalLeftPanel, 'bottomHorizontalLeftResizeBorder', hasContent, pendingHorizontalLeftHeight > 0 and pendingHorizontalLeftHeight or 200)
    setLeftHorizontalWidth()
  end

  if horizontalRightPanel then
    local hasContent = horizontalRightPanel:getChildCount() > 0
    setHorizontalPanelState(horizontalRightPanel, 'bottomHorizontalResizeBorder', hasContent, pendingHorizontalRightHeight > 0 and pendingHorizontalRightHeight or 200)
    setRightHorizontalWidth()
  end
end
...
function onPlayerUnload()
  ...
  for z, a in pairs(horizontalRightPanel:getChildren()) do
    -- Bug fix: the original predicate was `a.isOpen` (a field that nothing
    -- sets), so the table.insert below never ran and the horizontal docks
    -- always saved empty. Aligned with the vertical-sidebar loop above,
    -- which uses `a:isOpened()` (a method on the miniwindow base class).
    -- miniMap is a special case because it isn't a real miniwindow and
    -- isOpened() returns false for it — the vertical loop has the same
    -- carve-out.
    if a:getType() and (a:isOpened() or a:getType() == 'miniMap') then
      local tt = {type = a:getType()}
      if a.instance then
