...
function UIGameMap.create()
  ...
  if gameMap.setCursorAnimations then
    gameMap:setCursorAnimations(true)
  end
...
function UIGameMap:setContextCursor(cursor)
end

function UIGameMap:clearContextCursor()
end
...
function UIGameMap:markThing(thing, color)
  ...
  local markedColor = thing and thing.isMarked and thing:isMarked() and thing.getMarkedColor and thing:getMarkedColor()
...
function UIGameMap:getContextCursor(mousePosition)
  if not g_game.isOnline() or self:isDragging() then
    return nil
  end
  ...
  local mouseControl = m_settings.getOption('classicControl')

  if useThing then
    local lootControl = m_settings.getOption("lootControl")
    local canQuickLoot =
      (mouseControl == 1 and ((lootControl == 1 and keyboardModifiers == KeyboardNoModifier) or (lootControl == 2 and g_keyboard.isShiftPressed()) or (lootControl == 3 and keyboardModifiers == KeyboardNoModifier))) or
      (mouseControl == 2 and g_keyboard.isShiftPressed()) or
      (mouseControl == 3 and g_keyboard.isAltPressed())
    if canQuickLoot and ((useThing:isCorpse() and not useThing:isPlayerCorpse()) or useThing:inCorpse()) then
      return "quickloot"
    end
  end

  if lookThing and g_keyboard.isShiftPressed() then
    return "look"
  end

  if keyboardModifiers == KeyboardCtrlModifier and useThing then
  ...
  if g_keyboard.isAltPressed() then
    local creature = attackCreature or creatureThing
    if creature and creature ~= localPlayer then
      return creature:isNpc() and "talk" or "attacker"
    end
  end

  if keyboardModifiers == KeyboardNoModifier then
    if mouseControl == 3 then
      if creatureThing and creatureThing ~= localPlayer then
        return creatureThing:isNpc() and "talk" or "attacker"
      end
      if useThing then
        if useThing:isContainer() then
          return "open"
        elseif useThing:isMultiUse() or useThing:isUsable() or useThing:isForceUse() or useThing:isPickupable() then
          return "use"
        elseif useThing:isGround() or useThing:isGroundBorder() or useThing:isFullGround() or useThing:isIgnoreLook() or useThing:isNotPathable() then
          return "walk"
        end
      end
      return lookThing and "look" or "walk"
    end

    if attackCreature and attackCreature ~= localPlayer then
      return attackCreature:isNpc() and "talk" or "attacker"
    end
    if creatureThing and creatureThing ~= localPlayer and creatureThing:getPosition().z == autoWalkPos.z then
      return creatureThing:isNpc() and "talk" or "attacker"
    end
    if useThing then
      if useThing:isContainer() then
        return "open"
      elseif useThing:isMultiUse() or useThing:isUsable() or useThing:isForceUse() then
        return "use"
      end
    end
    return "walk"
