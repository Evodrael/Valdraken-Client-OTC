...
local __signalWrappers = setmetatable({}, { __mode = 'k' })

local function getWrappedSignalSlot(object, signal, slot)
  return slot
end

local function unwrapSignalSlot(object, signal, slot)
  local objectWrappers = __signalWrappers[object]
  if not objectWrappers then
    return slot
  end

  local signalWrappers = objectWrappers[signal]
  if not signalWrappers then
    return slot
  end

  return signalWrappers[slot] or slot
end
...
function connect(object, arg1, arg2, arg3)
  ...
  for signal,slot in pairs(signalsAndSlots) do
    slot = getWrappedSignalSlot(object, signal, slot)
...
function disconnect(object, arg1, arg2)
  ...
  for signal,slot in pairs(signalsAndSlots) do
    slot = unwrapSignalSlot(object, signal, slot)
    if not object[signal] then
    elseif type(object[signal]) == 'function' then
...
function TypingAnimation:start(id, label, text, speed)
    -- Defensive guard: callers (Prestige Arena, etc.) sometimes pass nil or
    -- an empty string when their data isn't ready yet, which would otherwise
    -- wipe whatever default text we just put in the label. Bail out and
    -- leave the previous animation/label content untouched in that case.
    if not label or text == nil or text == "" then
        return
    end
    ...
    -- Reset via the same accessor we use during ticks so we match the
    -- engine's text-rendering path (setColoredText, not the no-op
    -- setColorText stub).
...
function TypingAnimation:buildColoredText(parsedText, maxIndex)
    -- Emit setColoredText markup: `{text, color}` per contiguous color run.
    -- The C++ regex in UIWidget::setColoredText (src/framework/ui/
    -- uiwidgettext.cpp:405) only understands this brace syntax — the
    -- BBCode `[color=#hex]...[/color]` form is rendered literally.
    local result = ""
    local i = 1
    local last = math.min(maxIndex, #parsedText)
    while i <= last do
        local cd = parsedText[i]
        if cd.color then
            local color = cd.color
            local span = ""
            while i <= last and parsedText[i].color == color do
                span = span .. parsedText[i].char
                i = i + 1
            end
            result = result .. "{" .. span .. ", " .. color .. "}"
        else
            result = result .. cd.char
            i = i + 1
        end
    end
...
function TypingAnimation:processAnimation(id)
    ...
    -- UIWidget::setColorText is a no-op stub in this fork
    -- (src/framework/ui/uiwidget.h:595) — every binding-probe finds the
    -- function but it silently discards the input. The real method is
    -- setColoredText (with the D); use it whenever available so the typing
    -- animation actually renders incremental text with color tags.
    if instance.targetLabel.setColoredText then
        instance.targetLabel:setColoredText(currentText)
    elseif instance.targetLabel.setColorText then
...
function rgbToHex(color)
  ...
  return string.format('#%02X%02X%02X', color.r or 255, color.g or 255, color.b or 255)
end
