...
function UIMessageBox.display(title, message, buttons, onEnterCallback, onEscapeCallback)
  ...
  if type(message) == "table" then
    -- UIWidget::setColoredText (binding to a std::string_view) does not auto
    -- consume a {text, color, text, color, ...} table; convert it to the
    -- `{text, color}{text, color}` string format the C++ parser understands.
    if string and string.fromColoredTable then
      messageLabel:setColoredText(string.fromColoredTable(message))
    else
      messageLabel:setText(table.concat(message, " "))
    end
  else
