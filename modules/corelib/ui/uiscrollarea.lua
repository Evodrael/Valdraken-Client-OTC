...
function UIScrollArea:setVerticalScrollBar(scrollbar)
  ...
  if self.verticalScrollBar == scrollbar and self.verticalScrollHandler then
    return
  end

  if self.verticalScrollBar and self.verticalScrollHandler then
    disconnect(self.verticalScrollBar, { onValueChange = self.verticalScrollHandler })
  end

  self.verticalScrollBar = scrollbar
  self.verticalScrollHandler = function(scrollbar, value, delta)
  ...
  connect(self.verticalScrollBar, { onValueChange = self.verticalScrollHandler })
...
function UIScrollArea:setHorizontalScrollBar(scrollbar)
  ...
  if self.horizontalScrollBar == scrollbar and self.horizontalScrollHandler then
    return
  end

  if self.horizontalScrollBar and self.horizontalScrollHandler then
    disconnect(self.horizontalScrollBar, { onValueChange = self.horizontalScrollHandler })
  end

  self.horizontalScrollBar = scrollbar
  self.horizontalScrollHandler = function(scrollbar, value)
    local virtualOffset = self:getVirtualOffset()
    if not self.keepScrollRange then
  ...
  connect(self.horizontalScrollBar, { onValueChange = self.horizontalScrollHandler })
  self:updateScrollBars()
end
