...
function UIComboBox:onMousePress(mousePos, mouseButton)
  ...
  self.currentMenuWindow = menu
  menu:setId(self:getId() .. 'PopupMenu')
  for i,v in ipairs(self.options) do
    menu:addOption(v.text, function() self:setCurrentOption(v.text) end)
  end
  menu:setWidth(self:getWidth() + self.additionalWidth)
  menu:display({ x = self:getX(), y = self:getY() + self:getHeight() })
  menu:insertLuaCall("onDestroy")
  connect(menu, { onDestroy = function() self:setOn(false) self:setChecked(false) self.currentMenuWindow = nil end })
  self:setOn(true)
