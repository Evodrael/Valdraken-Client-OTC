...
function Options.createOrUpdatePreset(actionBar, slot, equipmentPreset, icon)
	...
	local presetCopy = table.recursivecopy(equipmentPreset)

	local foundEntry = false
	for _, data in pairs(Options.actionBarMappings) do
		if data["actionBar"] == actionBar and data["actionButton"] == slot then
			data["actionsetting"] = {}
			data["actionsetting"]["equipmentPreset"] = presetCopy
...
function Options.managePinnedCharacters(name, addCharacter)
  if not addCharacter then
    for i = #Options.pinnedCharacters, 1, -1 do
      local character = Options.pinnedCharacters[i]
  ...
  for _, character in pairs(Options.pinnedCharacters) do
    if name:lower() == character:lower() then
      Options.saveData()
      return
    end
...
local function ensureClientOptions()
  if not Options.array then
    Options.array = {}
  end

  if type(Options.array["options"]) ~= "table" then
...
function Options.setOption(option, value, dontSave)
  local options = ensureClientOptions()
