...
function Chat:onTabChange(tab)
    ...
    if tabMessage then
        ...
        if tabMessage:isServerLogChat() or tabMessage:isLocalChat() or tabMessage:isLootChannel() or tabMessage:isSpellChannel() then
...
function Chat:setup()
    ...
    -- Ordem fixa: Local Chat -> Server Log -> Loot, e SÓ DEPOIS os demais canais (Spell etc).
    local tab = self:addTabMessages(LOCAL_CHAT_NAME)
    self:addTabMessages(SERVER_LOG_NAME)
    local lootTab = self:addTabMessages(LOOT_CHANNEL_NAME)
    lootTab:setId(LOOT_CHANNEL_ID)
    ...
    self:addChannelConfig(LOOT_CHANNEL_NAME, LOOT_CHANNEL_ID)
...
function Chat:removeCurrentTab()
    ...
    if self.currentTab ~= LOCAL_CHAT_NAME and self.currentTab ~= SERVER_LOG_NAME and self.currentTab ~= LOOT_CHANNEL_NAME and self.currentTab ~= SPELL_CHANNEL_NAME then
...
function Chat:sendMapText(mode, creaturePos, name, message)
    if (mode == MessageModes.Say or mode == MessageModes.Whisper or mode == MessageModes.Yell or
        ...
        local staticMessage = message:gsub("{", ""):gsub("}", "")
        staticText:addMessage(name, mode, staticMessage)

        if mode == MessageModes.Potion then
            staticText:setColor(TextColors.orangeMob)
        elseif mode == MessageModes.NpcFromStartBlock or mode == MessageModes.NpcFrom then
