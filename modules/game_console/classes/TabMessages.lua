...
function TabMessages:setup()
    ...
    self.widget:setWidth(math.max(72, self.widget:getTextSize().width + 24))
...
function TabMessages:offline()
    self:stopSlowMode()
    self:clearMessages()
    if not self:isLocalChat() and not self:isServerLogChat() and not self:isLootChannel() and not self:isSpellChannel() then
