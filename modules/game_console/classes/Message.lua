...
local function buildMessagePrefix(message)
    local parts = {}

    if m_settings.getOption('showTimestampsInConsole') then
        table.insert(parts, os.date('%H:%M', message.timestamp))
    end

    if message.name ~= '' then
        local nameText = message.name
        if message.level > 0 and m_settings.getOption('showLevelsInConsole') then
            nameText = string.format('%s [%d]', nameText, message.level)
        end
        nameText = nameText .. ':'
        if #parts > 0 then
            table.insert(parts, ' ')
...
function Message:format()
    local prefix = buildMessagePrefix(self)
    if prefix ~= '' then
...
function Message:formatcolor()
    ...
    local prefix = buildMessagePrefix(self)
    if prefix ~= '' then
        setStringColor(text, prefix, color)
    end

    if type(self.text) == 'table' then
        if prefix ~= '' then
            setStringColor(text, ' ', color)
        end
        for i = 1, #self.text do
            table.insert(text, self.text[i])
        end
    else
        local body = prefix ~= '' and (' ' .. self.text) or self.text
        setStringColor(text, body, color)
    end
...
function Message:updateLabel(label, tab)
    ...
    label:setColoredText(string.fromColoredTable(self:formatcolor()))
...
function Message:highlightNPCChatText(label)
    ...
    label:setColoredText(string.fromColoredTable(highlightData))
