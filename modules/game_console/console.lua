...
function clearChannel(consoleTabBar)
  ...
  if not currentTab or not currentTab.tabPanel then
    return
  end

  local currentTabName = currentTab:getText()
  local consoleBuffer = currentTab.tabPanel:getChildById('consoleBuffer')
  if consoleBuffer then
    consoleBuffer:destroyChildren()
  end

  if readOnlyModeEnabled and currentTabName == activeactiveReadOnlyTabName and readOnlyPanel then
...
function toggleChat()
  ...
  local invisibleClick = consolePanel:getParent():recursiveGetChildById('invisibleClick')
  ...
  if chatToggleActive then
    ...
    local invisibleClick = g_ui.createWidget('ClickConsole', consolePanel:getParent())
...
function focusChat()
  if not g_game.isOnline() or not consolePanel then
    return
  end
  ...
  local parent = consoleTextEdit:getParent()
...
function onGameStart()
  ...
  -- Todo jogador entra com o Local Chat ja ativo. Feito com pequeno delay para rodar
  -- DEPOIS dos joins de canal assincronos (Loot/Spell/saved), que senao roubariam o foco.
  scheduleEvent(function()
    if g_game.isOnline() then
      g_chat:selectDefaultChannel()
    end
...
function onGameEnd()
  ...
  for k, v in pairs(g_chat:getTabsName()) do
    if not v:isLocalChat() and not v:isServerLogChat() and not v:isLootChannel() and not v:isSpellChannel() then
...
function onTalk(name, level, mode, text, channelId, pos, statement, groupId)
  print('LUA RECEIVED TALK:', name, mode, text)
  g_chat:onTalk(name, level, mode, text, channelId, pos, statement, groupId)
end
