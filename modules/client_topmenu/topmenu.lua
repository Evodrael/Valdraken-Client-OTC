...
local function bindTopLabel(widget, callback)
  if not widget then
    return
  end

  widget.onMouseRelease = function(self, mousePos, mouseButton)
    if self:containsPoint(mousePos) and mouseButton ~= MouseMidButton and mouseButton ~= MouseTouch then
      callback()
      return true
    end
  end
...
function init()
  ...
  if topMenu.discordLabel then
    ...
    bindTopLabel(topMenu.discordLabel, function()
      if topMenu.discordLink and topMenu.discordLink:len() > 0 then
        g_platform.openUrl(topMenu.discordLink)
      end
    end)
  end

  if topMenu.discord then
    bindTopLabel(topMenu.discord, function()
      if topMenu.discordLink and topMenu.discordLink:len() > 0 then
        g_platform.openUrl(topMenu.discordLink)
      end
    end)
  ...
  for index = 1, #Services.status do
    ...
    HTTP.postJSON(url, {type="cacheinfo"}, function(data, err)
      ...
      if apiCalls == #Services.status then
        if discordLink ~= '' then
          topMenu.discordLink = discordLink
        end
        if discordOnline ~= '' then
          topMenu.discordLabel:setTooltip(tr('Join Discord') .. '\n' .. discordOnline)
        end
